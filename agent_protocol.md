...
## Quick Reference（compact 恢复先读）

> ⚡ compact 恢复的最小集已抽到独立文件 **`docs/harness/quick_ref.md`**——角色/状态机/10 步/关键路径/关键规则/恢复三件套全在那。恢复时只加载 quick_ref.md + tasks_list.json + leader_checkpoint.md，省 token；要查完整规则再读本文件正文。
...
| `docs/harness_blueprint/specs/{feature}.md` | leader | 每 task 闭环时整理更新（当前生效规格） |
...
- **结构**：`docs/harness_blueprint/specs/{feature}.md`，按功能聚合（auth.md / user_settings.md / db_access.md...），不是 1 task 1 文件。
...
- **多 task 累积更新**：一个功能跨多个 task（如某功能被一个 task 建立、后续 task 加限流、再后续 task 做安全验收），后续 task 在同一 `docs/harness_blueprint/specs/{feature}.md` 追加/更新段落，不新建文件。
...
- **整理 docs/harness_blueprint/specs/{feature}.md**（强制：把本 task 当前生效规格整理进去，按功能聚合，多 task 累积更新）
...
- **隔离靠 leader 手动 git worktree**：每个并发 task 由 leader `git worktree add` 一个独立工作目录，Teams coder 在各自 worktree 工作，task 间天然隔离。**不用** Workflow 的 `isolation:'worktree'`——其粒度是 agent 不是 task，stage 间不可见（详见 `docs/harness/workflow_design.md` 决策 2）。
...
- 格式：按 task 分节，**节标题固定为 `## {TID} {title}`**（close_check.sh 用 `^## {TID}` grep 校验，不可改格式），表格列 `ID | 来源(review-code/review-test/环境) | 债项 | 严重度 | 暂存原因`。模板见 `docs/harness/template/harness_execution/tech_debt.md`。
...
- `docs/harness/quick_ref.md`（最小编排集，先读）+ 需要时再读本文件 `agent_protocol.md` 查完整规则
...
3. 读 quick_ref.md 重建编排逻辑；遇细节分歧再查 agent_protocol.md 正文
...
接口/参数/返回 + **脚本选择标准**：`docs/harness/workflows/README.md`；设计 why + 隔离决策：`docs/harness/workflow_design.md`。
...
- 并发隔离始终靠 leader 手动 git worktree，**不用** `isolation:'worktree'`（粒度是 agent 不是 task，详见 workflow_design.md 决策 2）
