...
**角色**：leader(opus, 主会话, 收口改共享文档) / coder(haiku, TDD) / reviewer(sonnet, 写 review_code.md) / test-reviewer(sonnet, 写 review_test.md)。无 doc-updater。
...
- FAIL 轮派三方（coder改 + reviewer/test-reviewer 重读反驳）
- commit 粒度=task，中间状态不 commit
- 并发=依赖分层+worktree 隔离，波次=DAG 层全部收口即结束
...
├── context.md        # coder 每轮追加：改了哪些文件、测试输出、假设
├── review_code.md    # reviewer 写：blockers/risks/verdict + coder 修改记录
└── review_test.md    # test-reviewer 写：假测试/缺失/verdict + coder 修改记录
```
...
- coder：读 review_*.md 正文 → 改代码 → 在同文件追加"修改记录"段（已改 X / 此项不改因为 Y / review 此处判断有误因为 Z）
