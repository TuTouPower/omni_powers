...
- **多 task 累积更新**：一个功能跨多个 task（如某功能被一个 task 建立、后续 task 加限流、再后续 task 做安全验收），后续 task 在同一 `docs/ref/specs/{功能}.md` 追加/更新段落，不新建文件。
...
6. 收口（见下"收口"段）
7. leader commit：类型按 task 性质选 feat/fix/refactor/docs，如 `feat({TID}): 简述`
