...
> 总指挥（leader, opus）编排 4 个常驻 teammate 完成 tasks_list.json 中的 T1-T32。
...
**单 task 10 步**：①确认 spec/plan 就位 ②拆 steps ③派 coder TDD ④派 reviewer+test-reviewer ⑤head -1 取 verdict（PASS→⑥ / FAIL→三方重读重审）⑥收口（progress/decisions/tech_debt/ref specs/tasks_list/归档）⑦commit ⑧回填 hash ⑨自检 compact ⑩下一个

**关键路径**：tasks_list.json=状态源 / docs/work/tasks/{TID}/=进行中 / docs/log/tasks/{TID}/=归档 / docs/ref/specs/{功能}.md=当前真相 / docs/work/leader_checkpoint.md=断点 / **docs/template/=所有文件模板（新建文件拷这里）**

**关键规则**：
- review 首行 `verdict: PASS/FAIL`，leader 只取首行不读正文
- FAIL 轮派三方（coder改 + reviewer/test-reviewer 重读反驳）；coder 修改记录写 review_*.md，禁碰 context.md
- commit 粒度=task，中间状态不 commit；step 不收口不单 commit；大到要多次收口 → 拆 task 派 task-splitter
- 并发=依赖分层+worktree 隔离，波次=DAG 层全部收口即结束；恢复后必须重算 DAG，不吃 checkpoint 惯性
- teammate idle = 可唤醒资源，FAIL/新 review 唤醒原实例不新 spawn
- spawn 失败重试2次→status=阻塞, blocked_by=spawn；回滚用 git revert 不用 reset

**恢复三件套**：本协议 + docs/work/tasks_list.json + docs/work/leader_checkpoint.md
...
doc-updater 角色砍掉——改 docs/ref/progress/decisions/tasks_list.json 都是共享文件，本就该走 leader 串行收口，中间隔子 agent 纯属多余。

**task-splitter 为什么单列**：拆 task 要读原 spec/plan 全文、切片、重写 tasks_list 片段——这堆中间内容若在 leader 上下文跑会大量挤占编排空间。它是确定性机械操作（不需要 opus 判断力），sonnet 足够。leader 只下"拆 T09 成 T09a/T09b，边界在 X"的指令，splitter 干完回报"已建目录/已切 spec/plan/tasks_list 已改"，leader 不读中间过程。**仅在拆分时启用，平时不存在**。

## 需求到 task 的前置流程

开发不是凭空开始，先有需求落 ref，再拆 task：

1. **改 ref**：用户需求进来，leader 先判断影响哪些真相文档（`docs/ref/prd.md`、`spec.md`、`architecture.md`、`domain.md`、`test.md`），按需更新。ref 是真相源，必须先于 task 同步。
2. **追加 tasks_list.json**：把需求拆成新 task（或修改现有 task），追加进 `docs/work/tasks_list.json`——每个 task 含 ID、标题、依赖列、验收标准、阻塞标记。
...
1. leader 判定拆分 + 定边界（哪些 step 归 T09a、哪些归 T09b、依赖关系），SendMessage 给 task-splitter
2. task-splitter 执行（全在自己上下文）：
   - 建 `tasks/T09a/`、`tasks/T09b/` 目录
   ...
   - 改 tasks_list.json：删原 T09 行，加 T09a/T09b（含它们之间依赖，通常顺序依赖），写依赖列/验收标准
...
**tasks_list 里原 task 可替换**："完成的 task 不删"针对已完成 task（保依赖链）；被重新 scope 的未完成 task 性质不同，删 T09 加 T09a/T09b，否则留个永不完成的 T09 误导选 task 规则。
...
第 3 轮仍 FAIL 不停下问用户。标 `docs/work/issues/{TID}_quality.md`，tasks_list.json 该行 status→阻塞，回到选 task 规则挑下一个。
...
- task 闭环后整个 `tasks/{TID}/` 目录挪到 `docs/log/tasks/{TID}/` 归档（git mv，保留历史）。
...
| `docs/work/tasks_list.json` | leader | 状态流转（task 清单 + 依赖 + status） |
| `docs/index.md` | leader | 文档导航总图（三态模型 + 目录索引），结构变动时同步 |
| `docs/ref/spec.md` | leader | 全局总纲 + specs/ 目录索引；需求变更时改 |
| `docs/ref/specs/{功能}.md` | leader | 每 task 闭环时整理更新（当前生效规格） |
| `docs/log/progress.md` | leader | 闭环后追加 |
| `docs/log/decisions.md` | leader | 有架构决策才追加 |
| `docs/work/tech_debt.md` | leader | 闭环后追加 MEDIUM/LOW |
| `docs/work/leader_checkpoint.md` | leader | 每 task 闭环后写，compact 恢复用 |
| `docs/work/issues/{TID}_*.md` | leader | 阻塞记录（`{TID}_quality.md` 质量阻塞 / `{TID}_spawn.md` spawn 失败） |

### docs/ref/specs/ 机制（当前真相源）

归档的 task spec 是历史快照，会过时，不维护。当前代码"是什么样"靠 `docs/ref/specs/`：

- **结构**：`docs/ref/specs/{功能}.md`，按功能聚合（auth.md / user_settings.md / db_access.md...），不是 1 task 1 文件。
- **docs/ref/spec.md 是目录**：只放全局总纲（技术栈/架构/安全边界）+ 指向 specs/ 各文件的索引。
- **task 闭环时整理**（强制，和 tech_debt 落盘同级）：leader 收口时判断本 task 改的功能对应 `docs/ref/specs/` 哪个文件，把 task spec 里**当前生效的规格**（接口/数据模型/约束/行为）整理进去。不是拷贝——过程性内容（方案比较/被否方案/自审）留在归档 task spec，docs/ref/specs/ 只留"现在是什么"。
- **多 task 累积更新**：一个功能跨多个 task（如 auth 被 T5 建、T26 加限流、T32 安全验收），后续 task 在同一 `docs/ref/specs/auth.md` 追加/更新段落，不新建文件。
- **归档 task spec 冻结**：`docs/log/tasks/{TID}/spec.md` 顶部盖戳"⚠️ 历史快照，以 docs/ref/specs/ 为准"，永不再改。

这样归档 spec 过不过时无所谓——当前真相全在 docs/ref/specs/，每 task 闭环同步，不漂移。
...
- reviewer：重读 review_code.md（看 coder 追加的反驳 + 新 git diff）→ 更新首行 verdict + 追加本轮意见
- test-reviewer：同上，重读 review_test.md

reviewer/test-reviewer 重读后：承认误判则首行改 PASS；维持原判则保持 FAIL 并追加理由。这样误判能在 1-2 轮内消除，不会白白耗满 3 轮。review 文档是双方来回对话，只追加不覆盖（首行 verdict 除外，每轮更新）。
...
- **leader 只取 verdict 首行**：`head -1 review_*.md` 或 `grep '^verdict:'`，不读正文。两份 PASS → 收口；任一 FAIL → 进 FAIL 轮，同时派 coder 改 + reviewer/test-reviewer 重读（详见 review 落盘规则）。
- PASS：leader 直接收口（改共享文档、提交）

teammate prompt 里只说"读 X 文件、写到 Y 文件、首行 `verdict: PASS/FAIL`"。文件名固定，不靠自然语言传话。

## Workflow 化（提案，落地后取代上面手工编排）
...
- PASS + leader 改完共享文档 + task 目录 git mv 到 docs/log/tasks/{TID}/ → commit
- message: `feat(T05): 昵称+密码注册登录`

**step 不是交付单元，不收口**：

- step 之间常互相依赖（step2 用 step1 接口），review 看整个 task 的 diff（所有 step 完成才派 review），单独 review 一个 step 没意义、多耗轮次。
- 收口动作全是 task 级（status→完成、归档目录、整理 ref/specs、写 checkpoint）。step 没做完整个 task，status 不能→完成、目录不能归档。"step 多次收口"语义自相矛盾。
- 回滚以 task 为粒度。一个 task 一个 commit = 一次 revert。多 commit 多收口 → 回滚要 revert 一串，下游也乱。

**大到要"多次收口" = 拆分粒度错了 → 拆 task，不是拆 commit**（见"拆 task"段）。

**WIP sub-commit 与收口脱钩**：大 task 跑很久、有中途崩溃丢进度风险时，允许 `wip(T09): step2` 性质的 sub-commit——**纯代码落盘，不触发任何收口动作**（不改 status、不归档、不写 checkpoint、不整理 ref）。收口时由 leader 定 squash 还是保留。这是上一版"task 内拆 sub-commit"的本意，关键是它和收口完全脱钩。
...
1. 写 `docs/work/leader_checkpoint.md`，内容：
...
- `docs/work/tasks_list.json`（状态）
- `docs/work/leader_checkpoint.md`（断点）
...
tasks_list.json 阻塞项 B1-B4 未解决前相关 task 跳过，标记 `status=阻塞, blocked_by={原因}`：

| 阻塞 | 影响 task | 处理 |
|---|---|---|
| B1 平台模型 key | T8,T16,T18,T19,T20 | 跳过，标"status=阻塞, blocked_by=key" |
| B2 搜索 key | T17 | 跳过，标"status=阻塞, blocked_by=key"（T18 依赖搜索但用 mock 可过） |
| B3 域名 | T28,T31,T32 | 跳过，标"status=阻塞, blocked_by=domain" |
| B4 反馈链接 | T24 | 用占位 `https://example.com/feedback`，记 tech_debt，不阻塞 |

第 3 轮 review 仍 FAIL → 写 `docs/work/issues/{TID}_quality.md`，标"status=阻塞, blocked_by=quality"，跳过继续。
...
2. 仍失败 → 该 task 标"status=阻塞, blocked_by=spawn"（`blocked_by: "spawn"`），写 `docs/work/issues/{TID}_spawn.md` 记录错误
...
4. 若该 task 已归档到 `docs/log/tasks/{TID}/`，git mv 回 `docs/work/tasks/{TID}/`
5. 重新进入开发循环
...
- 查依赖是否完成：`jq '.tasks[] | select(.id=="T05") | .status'`
...
- `docs/work/tasks_list.json`：只留未完成（待开始/进行中/审阅中/阻塞）
- `docs/log/tasks_done.json`：已完成 task，裁剪到最小（id/title/dependencies/commit，删 verification）
- 依赖检查：活表查不到的依赖 → 查 done 表确认完成
- 收口时 task 从活表移到 done 表

32 个 task 量级不需要拆，靠查询即可。
...
```
1. leader 确认 tasks/{TID}/ 的 spec.md + plan.md 已就位（由"需求到 task 的前置流程"生成，此处不重新生成）
   （并发时：在各自 worktree 里）
2. leader 读 plan，拆 step 列表存 tasks/{TID}/steps.md
3. leader 按"上下文管理·teammate 层"的 coder 阈值规则决定复用/重 spawn，派活：
...
4. coder 全部 step 完成 → **leader 验收最小标准**：tasks/{TID}/context.md 存在且非空 + 对应 tests/ 有新增/修改文件。缺任一 → 退回 coder 补，不派 review。
   验收通过 → leader 检查两个 reviewer 的上下文（≥70% 则让其 compact，否则直接派活给常驻实例），派 reviewer + test-reviewer 并行：
   - reviewer: 读 git diff + context.md → 写 review_code.md（首行 verdict）+ SendMessage
   - test-reviewer: 读 tests/ + context.md → 写 review_test.md（首行 verdict）+ SendMessage
   （并发时：常驻 reviewer 串行连审多个 task，或多套 reviewer；产出落各自 review_*.md 互不干扰）
5. leader `head -1` 取两份 review 首行 verdict（不读正文）：
   - 都 PASS → step 6
   - 任一 FAIL → 进 FAIL 轮，同时派三方：
     - coder 读 review 正文 → 改代码 → 在 review_*.md 追加修改记录段（禁止写 context.md）
     - reviewer 重读 review_code.md（coder 反驳 + 新 diff）→ 更新首行 verdict
     - test-reviewer 重读 review_test.md → 更新首行 verdict
     → leader 再取首行 verdict 判断（max 3 轮）
   - 第 3 轮仍 FAIL → 标"status=阻塞, blocked_by=quality"，写 issues/{TID}_quality.md，跳过该 task
6. 收口（见下"收口"段）
7. leader commit：类型按 task 性质选 feat/fix/refactor/docs，如 `feat(T05): 昵称+密码注册登录`
8. leader 在 progress.md 回填 commit hash
9. 自检 compact（每 3 task 强制，或可疑时）
10. 串行：回选 task 规则挑下一个；并发：波次内全部收口后开下一波次
```
...
- **整理 docs/ref/specs/{功能}.md**（强制：把本 task 当前生效规格整理进去，按功能聚合，多 task 累积更新）
- tasks_list.json 该 task status→完成
- 按需更新 docs/ref/（架构/约定受影响时）
- 归档 task spec 顶部盖戳"历史快照，以 docs/ref/specs/ 为准"
- git mv tasks/{TID}/ → docs/log/tasks/{TID}/（归档，不删）
...
| ID | 来源 | 债项 | 严重度 |
|---|---|---|---|
| T5-1 | review-code | 前端 key 输入框明文 | MEDIUM |
| T5-4 | 环境 | 真实 DB 验收未跑 | HIGH |
```

leader 收口 grep `## tech_debt` 到下一个 `---`/EOF 之间的表格，追加到 docs/work/tech_debt.md。

### 并发的额外约束（仅波次宽度>1 时生效）

- **隔离**：每个并发 task spawn coder 时用 `isolation: "worktree"`，coder 在独立 git worktree 工作，互不可见。
...
- **并发上限**：`min(同层可跑 task 数, 3-4)`，依 teammate 通信稳定度调整。
