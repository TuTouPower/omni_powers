...
@test "op_checkpoint: TID 锚定——T01 不误配 T010" {
  setup_mock_project
  jq '.tasks += [{"id":"T010","title":"other","status":"done","spec":"specs/T010_x.md","depends_on":null,"workset":[],"eval":"skip","eval_reason":"fixture"}]' docs/omni_powers/op_execution/tasks_list.json > /tmp/tasks.json && mv /tmp/tasks.json docs/omni_powers/op_execution/tasks_list.json
  run "$OP_HOME/skills/oprun/scripts/op_checkpoint.sh" T01
  [ "$status" -eq 0 ]
  count=$(grep -cE '^- T01 ' docs/omni_powers/op_execution/leader_checkpoint.md)
  [ "$count" -eq 1 ]
