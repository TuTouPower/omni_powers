...
@test "op-checkpoint: 追加已完成 task（首次）" {
  setup_mock_project
  run "$OP_HOME/skills/oprun/scripts/op-checkpoint.sh" T01
  [ "$status" -eq 0 ]
  grep -qE '^- T01 "test task" ✅ ' docs/omni_powers/op_execution/leader_checkpoint.md
  teardown_mock_project
}

@test "op-checkpoint: 幂等——重跑不重复（P1-7）" {
  setup_mock_project
...
@test "op-checkpoint: TID 锚定——T01 不误配 T010" {
  setup_mock_project
