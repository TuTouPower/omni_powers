...
全线 Sub Agent（D15）。每次 task 重新 dispatch，上下文隔离。（D16）所有 agent 共用一个工作目录。
...
- `docs/op_execution/issues/{TID}_quality.md` 记录质量阻塞（3 轮 FAIL）。

### 持久文件

| 路径                                       | 谁写   | 何时                                                |
| ------------------------------------------ | ------ | --------------------------------------------------- |
| `docs/op_execution/tasks_list.json`      | op-closer / leader | 状态流转                                         |
| `docs/op_blueprint/specs/{feature}.md`   | op-closer / leader | 每 task 闭环整理（当前生效规格，按功能聚合）        |
| `docs/op_record/progress.md`             | op-closer / leader | 闭环后追加                                          |
| `docs/op_record/decisions.md`            | op-closer / leader | 有架构决策才追加                                    |
| `docs/op_execution/tech_debt.md`         | op-closer / leader | 闭环后追加                                          |
| `docs/op_execution/leader_checkpoint.md` | leader | 每 task 闭环后写                                    |
| `docs/op_execution/dag.md`               | leader | 每次 /op-start 从 depends_on 重算生成               |
| `docs/op_blueprint/` 下其他文档          | op-closer / leader | 按需更新                                          |

（D16）串行模式下无并发竞争，closer 可直接更新 tasks_list.json 和 specs/ 等持久文件。leader 审查 closer 产出后 commit。
...
**整理规则**：每 task 闭环时，把 task spec 里当前生效的接口、数据模型、约束、行为整理进对应功能 specs 文件。同一功能跨多个 task 时累积更新同一个文件。归档后永不再改。
...
每个 task 的 `depends_on` 记录其前置依赖（数组，无依赖则 `null`）。每次 `/op-start` 从 `depends_on` 重算拓扑分层，生成 `docs/op_execution/dag.md`。
