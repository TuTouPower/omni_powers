...
每个 task 的 `depends_on` 记录其前置依赖（数组，无依赖则 `null`）。每次 `/op-start` 从 `depends_on` 重算拓扑分层，生成 `docs/omni_powers/op_execution/dag.md`。DAG 可帮助 leader 识别多个 `待规划` task 并行补 spec/plan；进入代码执行后仍按可跑 task 单个串行推进。
