...
BMad Method（v6.9.0）是一个面向 AI 编程 Agent 的结构化开发方法论与工作流系统。通过 **命名角色 Agent、分阶段工作流、渐进式上下文构建** 三重机制，将模糊的产品意图转化为可实施的代码，全程由 Agent 自行编排而非人工管理。
...
|------|------------|-------------|
| 安装方式 | CLI 工具复制技能到项目 | git clone + install.sh symlink |
| 配置注入 | 不修改 settings.json/CLAUDE.md | 通过 SessionStart hook 注入 |
| Agent 模型 | 命名角色 persona（personality-driven） | 功能角色（op-coder、op-code-reviewer） |
| 状态持久化 | memlog.py + manifest + sprint-status.yaml | 无内置持久化 |
| hooks 使用 | 不使用 hooks | SessionStart 注入 |
| IDE 适配 | 17+ IDE | Claude Code only |
| 模块系统 | 官方模块注册表 + marketplace plugin | 无 |
| SessionStart 注入 | 无（按需加载） | 有（每次会话注入） |
| 工作流控制 | step-file 架构 | 子 Agent 收口模式 |
| 上下文消耗 | 低（渐进加载，2K-5K tokens 起步） | 较高（全局 CLAUDE.md 持续加载） |
| 语言支持 | 多语言（含中文文档翻译） | 中文 |
