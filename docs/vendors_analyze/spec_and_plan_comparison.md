...
| **OpenSpec** | SPEC（delta spec）→ tasks → implement（**无 plan**） | **强制** | **无 plan 阶段** | Delta spec：ADDED/MODIFIED/REMOVED 差异描述 | 存在 |
...
- `inject-subagent-context.py` 自动向 implement/check/research 子 agent 注入 prd.md + design.md（可选）+ implement.md（可选）
