...
├── e2e/                             # 【代码，永久资产】**路径由 config.OP_E2E_DIR 定**——init 时问用户放哪，默认 heavy=顶层 `e2e/`、lite=`docs/omni_powers/e2e/`；**用户项目已有顶层 e2e/ 时 init 探测提示**（迁移子目录 / 显式豁免进保护 / 换路径），避免用户既有测试被锁
...
│   ├── config                       # 项目级路径配置：`OP_E2E_DIR=...`（相对项目根），脚本读此不硬编码；init 时由用户定
...
免派 task 在 tasks_list 标 `eval: skip`（或 spec frontmatter 标注），oprun 跳过 dispatch evaluator，直接进 closer。其余 task（有用户可观察行为 AC）一律派。
...
**副本同步淘汰**：`scripts/build_lite.sh` 及其校验逻辑删除——共享目录单份，无副本漂移问题。
...
| spec 写保护 + merge gate 拦受保护路径（§3.4） | lite 无分支拓扑，无 merge gate；**spec 写保护机械校验**：oplrun 收口前对 `op_execution/specs/**` 跑 `git diff` 非零即停——从"约定"升"机械"，堵 implementer 主分支直改 spec 致同源污染 |
