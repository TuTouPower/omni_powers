...
7. **证据由机器产出——"机器"必须在被监督者控制之外。** hook 自动跑测试留原始输出这条路对 subagent 已失效（D18：PostToolUse 对 subagent 不触发），implementer 的测试输出实际是它自己跑 Bash 产生、自己写进 report 的——**当前 per-task 证据可被 implementer 伪造**（SubagentStop 只能验"存在新鲜输出文件"，验不了真伪）。诚实结论：过渡期 per-task 证据链的可信度靠 reviewer 双裁决 + Stage 4 独立验收兜底，不靠"机器证据"三个字。**目标形态（P2）：per-task 证据 CI 化**——implementer 分支 push 触发 CI 跑测试，结果以 CI 为准（agent 无法篡改 CI 结果），与 evaluator 构建产物链路共用同一套 CI 基建（§10 第 3 层）。"bash 先算状态，LLM 再决策"不变——凡确定性计算（工作集核算/状态判定/diff 打包）都交脚本，不留给模型。
...
- 降细拆惩罚：①brief 附定向包（architecture.md + conventions 摘要；lite 无 blueprint，定向包退化见 §14）；②**接口先行排序**；③接受部分重复读入是买来的干净上下文。
...
2. **op-closer 只做这两件事**：将 reviewer 标【暂存】的问题确定性写入 `op_execution/issues/`（加 `tech-debt` 标签，不二次筛选）+ append 本 task 的架构/自决决策到 `op_record/decisions.md`（append-only，不经 leader 审批）。注：decisions.md 是**多写者**文件（红灯归因/解锁、leader 降级 delta、closer 收口均按来源标记 append）——多写者不冲突的前提是 task 串行（原则 9），并行写 append-only 文件是 git 合并最差场景。
...
> 下表是**推荐档**（design 建议）；实际以 `OP_*_MODEL` 环境变量为准，未设则继承主会话当前模型（本节末段）。token 消耗排序：implementer > evaluator > reviewer > closer。
...
1. **结构隔离层（目标硬底线，当前未落地）**：目标形态下 evaluator 在独立 worktree 工作，文件系统只挂载 spec + 生效规格（开工前基线）+ baselines 索引 + 应用启动方式 + 构建产物（Electron 可执行文件 / web dist / 扩展 .zip / 服务二进制）+ `e2e/`。源码 `src/**`、task 目录（`op_execution/tasks/**` + `op_record/tasks/**`）、`op_record/decisions.md` 物理不挂载——结构上不可能读，纪律禁止次要。implementer 分支跑 CI 构建产出打包好的应用供 evaluator 操作（与 per-task 证据 CI 化、e2e 只读信号回传共用同一套 CI，原则 7 / §2）。
   ...
   - ⚠️ **实现状态**：当前 oprun 创建普通 worktree（`git worktree add`），未用 sparse-checkout 排除 `src/e2e`——硬底线的**实现待 §12 P2 落地**。过渡期靠 agent 提示词 + 路径约定（advisory，非物理强制）；不得把当前 worktree 当物理隔离安全前提（§0.1）。
...
4. **刻薄化调教循环**（持续）：
   ...
   - ⚠️ 钓鱼审计依赖「独立验证环境副本」基建（独立构建链路 + 副本管理脚本），P3 与独立验证环境增强一并落地（§12）；基建就绪前钓鱼收敛判据暂缓生效，防放水靠前三层。
...
### 8.3 BUG-*/e2e 合法写入与 git 硬锁的交互（开放问题，P2 设计项）

git pre-commit 硬锁 `e2e/**`，但存在合法写入路径：evaluator 固化 PASS 测试、leader 转交 BUG-* patch、closer 提案后 leader 改跨功能既有 e2e。git hook 层分不清「谁」在提交（同样面临 D18 式身份识别困境）。候选方向：①合法写入统一走 leader 主会话执行（pre-commit 白名单一个受控入口，例如带特定 commit trailer + 解锁脚本配对）；②e2e 写入走独立分支 + 受保护合并。**具体机制 P2 实现 worktree 隔离时一并定**，此处仅登记问题，防止实现时把「硬锁」做成「evaluator 也被锁死」或「解锁通道形同虚设」两个极端。
...
防"偷偷改绿"按强度排序。⚠️ **前提**：PreToolUse deny 对 subagent 整体失效（§8.1 前提，依据见 D18），implementer/evaluator 均为 subagent——故以下防线对 subagent 场景不依赖 hook 拦截，主防线靠**结构 + git + CI 层**；hook 拦截仅主会话 leader 场景有效，作 advisory。第 1-5 层是 hook/结构防线，第 3'/6 层是独立 CI 任务（非 Claude Code hook）。

1. **结构隔离 + 行为层保护**（worktree 对称 + git 层，目标硬底线，当前过渡期 advisory）：`e2e/**` 对 implementer 的目标保护是 **worktree 对称隔离**——implementer worktree 不挂 `e2e/`（§2）；当前普通 worktree 未排除目录，禁止主动读写但非物理强制。`BUG-*` 新增属 fix 流程（带归因+解锁审批，§1.2），由 evaluator 验收时写或 implementer 产 patch 由 leader 转交，implementer 不直接落盘 `e2e/`；既有 `e2e/`+`BUG-*` 修改硬锁靠 **git 层**（branch protection + pre-commit/post-receive hook 在 git 操作层，非 Claude 工具层——绕过 §8.1 的 subagent deny 失效；合法写入通道与硬锁的交互见 §8.3 开放问题）。归因记 decisions.md。
2. **访问隔离**（结构性，详见 §8.1）：evaluator 仅接触 spec + 构建产物 + baselines，不接触源码或 implementer 交接文件——靠 worktree 无 src 实现（非 hook）。
3. **机器证据**（过渡期 hook advisory，目标 CI 化——原则 7）：
   - **过渡期（当前）**：PostToolUse[src/**] 自动跑受影响测试仅主会话场景触发；subagent 场景靠 **SubagentStop**（非主会话 Stop——拦 implementer subagent 交工，matcher 按 agent_type 过滤）检查 tasks_list.json 状态 + 新鲜测试输出，缺则拒收工。**诚实边界：SubagentStop 只验"存在"，验不了"真伪"**——implementer 自跑自贴的输出可伪造，可信度靠 reviewer 双裁决 + Stage 4 兜底。hook 脚本开头**必检查 stdin 的 `stop_hook_active` 字段**，为 true 时 exit 0 防递归无限循环；禁 --no-verify。结构层单测明确不设防，由行为层兜住。
   - **目标（P2，CI 化）**：implementer 分支 push 触发 CI 跑测试 + **只读跑 e2e 全集回传结果**（§2，给集成信号不给写权），reviewer/leader 以 CI 结果为准——agent 无法篡改 CI 输出，证据链脱离被监督者控制。与 evaluator 构建产物链路（§8.1 第 1 层）共用同一套 CI。
4. **spec 写保护**（git 原生为主）：approved/in_progress 受 **git 层保护**（pre-commit hook 拦 spec 路径变更，走变更子流程）；主会话 leader 场景额外 PreToolUse 拦截（subagent 场景失效）；SessionStart 注入前 git diff --quiet 校验，防"好心更新规格"漂移。
5. **警告+留痕**（advisory 兜底）：结构层测试编辑按行敏感度——import/setup/调用行静默；expect/assert 行强制说明理由。⚠️ 仅主会话场景生效，subagent 场景 deny 被忽略（曝光价值在主会话；subagent 靠 reviewer 双裁决 + 结构层兜）。危险模式：删除/反转 expect、toBe→toContain/正则/>=、timeout/阈值增大、.skip/.only、删测试文件或 it 块、test 文件加 eslint-disable。价值在曝光不在阻止。
6. **定期体检**（每周/CI 异步，**非 Claude Code hook——独立 CI 任务**）：skip/only 计数、timeout 增幅、恒假断言、纯存在性断言 E2E；触碰 INV 模块抽样跑变异测试（杀不死变异体的测试判假重写）。产出落 issues。

**防线层 ↔ 实现手段映射**（⚠️ 读者注意：主防线 1/2/3-目标层**不在** §11 hook 清单里——配齐 hook ≠ 安全，hook 对 subagent 失效，主防线靠结构、git 与 CI；当前安全水位见 §0.1）：

| §10 防线层 | 实现手段 | §11 Claude hook？ |
|---|---|---|
| 1 结构隔离 + 行为层保护 | 目标：worktree 对称（无 e2e/）+ git branch protection / pre-commit / post-receive；当前：worktree 未排除，advisory | ❌ 非 hook（结构 + git） |
| 2 访问隔离 | 目标：evaluator worktree 无 src（§8.1）；当前：worktree 未排除，advisory | ❌ 非 hook（结构） |
| 3 机器证据 | 过渡期：PostToolUse[src/**] + SubagentStop（验存在不验真伪）；目标：**CI 跑测试+e2e 只读信号**（P2） | 🟡 过渡期 hook / 目标 CI |
...
scripts/（确定性计算全归 bash，不留给模型；lite 自带副本见 §13.4）
...
**P0（第一周，零基建）**：install.sh + opinit/oplinit 双 init；入口分拣两判定 + spec 模板与命名约定（含可测性契约）+ 闸门 A + 审批即 commit；任务卡 + task=commit；红灯归因、可写性矩阵、review 两轮上限、契约边界规则进 RULES.md/test.md；完成必须贴测试输出；issues 手工登记。**worktree 无 src 工程 spike 启动**（evaluator/implementer worktree 挂载范围 + CI 构建产物生产链路可行性，§8.1 前提，硬底线必须结构隔离）。
...
4. **上下文水位检查**：oplrun 每 N task（如 3）自检 leader 上下文水位，逼近阈值时提示 `/compact` 或建议转 heavy——否则跑到第 8 task 静默失能。这是 lite「自验代 hook」的根本张力，必须有检测机制，不能默默崩。（heavy leader 干的编排事更多但不亲读证据，靠 checkpoint+compact 恢复容错；lite leader 亲验证据，水位是硬约束）
...
>
> **本合并版新增决策（2026-07-07，尚未实现，随 §12 排期）**：
> - per-task 证据 CI 化（原则 7，P2）
> - implementer 分支 e2e 只读信号回传（§2，P2）
> - task 严格串行、任务卡去「可并行」字段（原则 9，文档生效）
> - 闸门 A 预算改 15-30 分钟/叶子（原则 11，文档生效）
> - lite P0 阻断检查（§14.2，待落 oplrun）
> - op_blueprint 占位 README（§14.3，待落 oplinit）
> - spec 前缀双字母编码（§3，文档生效）
> - e2e 合法写入与 git 硬锁交互登记为开放问题（§8.3，P2 设计项）
>
> **用户裁决（累计）**：① lite 复用 heavy 三区布局；② agent 装 `~/.claude/agents/`；③ 接受 evaluator 裸评退化；④ 脚本自包含（方案 B）；⑤ e2e/ 用项目根；⑥ lite close 砍 op_close_pre；⑦ 先改文档标分界再实现；⑧ 统一 install.sh 全量装、按项目 init 分轻重；⑨ 证据链 CI 化 + e2e 只读信号 + 严格串行 + 闸门 A 15-30 分钟 + lite P0 检查（本版决策 1B/2B/3A/4A/5B）。
>
> 完整审阅意见处置史（三轮审阅采纳/驳回明细）见 `docs/archive/omni_powers_lite_design.md` §15。
