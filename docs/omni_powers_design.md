...
>
> **两模式一份设计**：heavy（全量：task 分支拓扑 + merge gate + worktree 隔离 + blueprint 真相源 + hook advisory）与 lite（低侵入：不加 hook、无分支拓扑，仅写项目 env.OP_DOCS_DIR，不改用户已有文档）。执行内核（spec/验收标准/不变量/TDD/双裁决/验收/状态机/角色职责）两版共享；差异收敛在环境集成层（§5）。安装统一走仓库 `install.sh` 一次装齐 `~/.claude/`，**按项目 init 选模式**：`/opinit`=heavy，`/oplinit`=lite，同一项目只认一个 profile。
...
> **安装模型**（非 Claude Code plugin 市场机制）：用户 git clone 本仓库 → 跑 `bash install.sh --set-ophome` **一次装齐**两模式全部 skill+agent 进 `~/.claude/` + 写 `OP_HOME` 到 `~/.claude/settings.json` 全局 env（两版都需要——脚本统一在 `$OP_HOME/scripts/`）。`--link` 开发模式软链，改仓库即生效。

**install.sh 职责边界**：只装 skill + agent + 写 `OP_HOME` 全局 env。不装 scripts 到 `~/.claude/`，不碰项目级配置。

**项目初始化**：项目内跑 `/opinit`（heavy）或 `/oplinit`（lite）。同一项目只认一个 profile（§5.2）。

**heavy `/opinit`**：三区骨架 + profile=heavy + hooks 注册 + 建 `tests/e2e/`。**可重构项目 `CLAUDE.md`**（插入 omni 流程引导段、skill 入口说明、目录结构）。

**lite `/oplinit`**：三区骨架 + profile=lite。低侵入——不加 hook、不改造项目文件结构，仅写项目 `.claude/settings.json.env.OP_DOCS_DIR`。**不修改项目 `CLAUDE.md`**。建 `$OP_DOCS_DIR/e2e/` 作为 lite E2E 默认落点；当 `$OP_DOCS_DIR=docs` 时，共享 README/index/.gitignore 仅维护 managed block。
...
对外 skill（7）——用户心智模型："装一次、每项目 init 一次、进料、跑、看"
  heavy 入口（3）:
    ...
    opintake    需求入口：分拣 → spec（含设计探索+可测性契约）
                → 拆 task（awaiting_gate）→ 闸门 A（→ready）→ tasks_list.json 写入 `status=ready` 的 task ══ 终点：task ready（待开始）
    oprun       从 checkpoint 续跑：task 循环（review → merge → per-task 验收 → closer 收尾 → 闸门 C → 归档）
                leader 即 controller，被本 skill 驱动
  lite 入口（3）:
    oplinit     lite 初始化（一次性）：三区骨架 + profile=lite（低侵入：不加 hook，仅写 env.OP_DOCS_DIR，不改已有文档）
    oplintake   需求入口：spec + 拆 task + 闸门 A
...
**允许**：
- `~/.claude/` 全局安装（`install.sh --set-ophome`）：skill/agent/OP_HOME env——用户级配置，不算项目侵入
- 项目内 `$OP_DOCS_DIR/`（含 lite `e2e/` 验收资产）：默认新增独立子目录；选择共享 `docs` 时只维护 managed block，不覆盖宿主内容
- 项目 `CLAUDE.md` 末尾插入说明段（告知 `docs/omni_powers/` 目录用途 + e2e 落点 + `/oplintake`/`/oplrun`/`/opstatus` 入口），不删不改原有内容；用户拒绝则跳过

**禁止**：
- 修改项目 `.claude/settings.json`（不注册 hook）
- 重构/删改项目已有文件
- 创建 task 分支、merge gate 等文件结构变更
...
- agent 派发机制两版相同：`subagent_type: "op-implementer"`。**agent markdown 是静态文件，两版共用一份**。
