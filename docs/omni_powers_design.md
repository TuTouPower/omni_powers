...
OP 独占资产：`op_blueprint/`、`op_execution/`、`op_record/`、`profile`，lite 另含 `$OP_DOCS_DIR/e2e/`。当根为共享 `docs` 时，README.md、index.md、.gitignore 只维护 omni_powers managed block；迁移和卸载必须保留宿主内容。heavy 行为层 E2E 仍位于仓库 `e2e/`、`tests/e2e/` 或 `tests/*/e2e/`，不随 OP 根迁移。
