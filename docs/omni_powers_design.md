...
项目运行资产根由 `.claude/settings.json.env.OP_DOCS_DIR` 指定。解析优先级：显式参数 > 项目 settings > 进程 env > 默认 `docs/omni_powers`。`/opinit`、`/oplinit` 首次一次询问，支持 `docs` 或其他安全项目相对路径；旧项目无配置继续默认根。

路径层统一由 `$OP_HOME/scripts/op_paths.sh` 提供。路径必须 repo-relative，拒绝绝对路径、`.`/`..`、`.git`、`.claude`、空白/控制字符、glob/pathspec magic。settings 损坏或类型非法时 fail closed。下文未特别标注的 `docs/omni_powers/` 均表示默认值；运行时契约写作 `$OP_DOCS_DIR/`。

OP 独占资产：`op_blueprint/`、`op_execution/`、`op_record/`、`profile`、`op_readme.md`、`op_index.md`，lite 另含 `$OP_DOCS_DIR/e2e/`。当根为共享 `docs` 时，只有 `.gitignore` 继续使用 omni_powers managed block；迁移和卸载必须保留宿主内容。旧版 `README.md`/`index.md` 中 managed block 仅作为升级与卸载兼容输入，脚本迁移后以 `op_readme.md`、`op_index.md` 为独立完整文件。heavy 行为层 E2E 仍位于仓库 `e2e/`、`tests/e2e/` 或 `tests/*/e2e/`，不随 OP 根迁移。

> **定位**：设计档案——系统怎么设计及为什么。不进运行时上下文。运行时操作见 `$OP_HOME/RULES.md`，agent 行为见 `agents/*.md`，skill 流程见 `skills/*/SKILL.md`。
>
> **两模式一份设计**：heavy（全量：task 分支拓扑 + merge gate + worktree 隔离 + blueprint 真相源 + hook advisory）与 lite（低侵入：不加 hook、无分支拓扑，仅写项目 env.OP_DOCS_DIR + bind 项目 skill，不改用户已有文档）。执行内核两版共享；差异收敛在环境集成层（§5）。`install.sh` 写 OP_HOME + 全局仅 `/opinit`/`/oplinit`；业务 skill 在项目 init 时 bind。同一项目只认一个 profile。
...
| **merge gate（写入硬底线）** | leader 唯一主分支写入口 + **白名单**机械校验（task 分支允许触碰 = workset ∪ `tasks/{TID}/report.md` ∪ 结构层测试路径，其余 REJECT，§3.4）+ review verdict 读主分支 review.md 末行 | **已落地**（`op_merge_gate.sh`） | 硬（被监督者之外，信任根声明 §0.1） | 受保护路径变更混入主分支 |
...
```
<project>/
├── CLAUDE.md                        # heavy：请求用户批准启用 omni_powers，批准后新增一行执行 docs/omni_powers/op_index.md；**lite 不改此文件，靠 docs/omni_powers/profile 发现（§5.2）**
├── tests/e2e/                        # 【代码，永久资产】heavy E2E 落点（默认 `tests/e2e/`，init 时问用户确认）；lite 默认 `$OP_DOCS_DIR/e2e/`（§5.3）
├── docs/omni_powers/               # 三区根目录（op_blueprint/op_execution/op_record）
│   ├── op_readme.md                 # 给人看的入口概览
│   ├── op_index.md                  # 给 agent 看的目录页（heavy: /oprun 启动读其摘要）
...
| `awaiting_gate` | 待闸门 A 审批 | draft spec 就位 + task 已拆，待人批；oprun 不领 | §2.2 闸门 A 前 |
| `ready` | 待开始 | 闸门 A 已批，spec approved | §2.2 闸门 A 后 |
...
| 文档 | 唯一职责 | 不该有（指向即可） |
|---|---|---|
| `CLAUDE.md`（项目入口，heavy 管理路径） | 项目一句话定位 + dev/build/test 命令 + 指向 `op_blueprint/` 各文档；**lite 不改此文件，入口为 profile + op_index.md** | 技术栈/目录树/架构约束/命名/日志/调试规则 |
...
免派 task 在 tasks_list 标 `eval: <免派理由>`（**task schema 单字段 `eval`，D9**——值 `"required"` = 派 evaluator，其余值 = 免派理由文本（接口先行/脚手架/纯内部重构，见上判据）；oprun 机械判定 `eval == "required"` 即派，非临场判断；spec frontmatter 亦可标），oprun 跳过 dispatch evaluator，直接进 closer。其余 task（有用户可观察行为 AC）一律派。
...
### 3.4 git 拓扑与主分支回流协议（merge gate——写入硬底线所在；**已落地：`op_merge_gate.sh`，用 merge-base 三点 diff，不依赖 dispatch 锚点**）
...
> **安装模型**（非 Claude Code plugin 市场机制）：`bash install.sh --set-ophome` → 同步资产到 `OP_HOME` + 写 `env.OP_HOME` + **全局仅软链 `opinit` 与 `oplinit`**。业务 skill（oprun 等）在 `/opinit` 或 `/oplinit` 时由 `op_bind_project_skills.sh` 软链到**项目** `.claude/skills/`。一律 `disable-model-invocation: true`。**agent 永不注册**发现路径——`$OP_HOME/agents/*.md` 作提示词模板，派发 `Read` 注入 `general-purpose`。`--link` 开发模式同步 OP_HOME 用软链。

**install.sh 职责边界**：同步 OP_HOME + 全局只装两个 init skill + 写 `OP_HOME` env + 清理旧版全局业务 skill/agent。不装 scripts 到 `~/.claude/`，不碰项目（除用户随后跑 init）。

**项目初始化**：`/opinit`（heavy）或 `/oplinit`（lite）——**第一步 bind 项目 skill**，再骨架。同一项目只认一个 profile（§5.2）。

**heavy `/opinit`**：bind heavy 集 + 三区骨架 + profile=heavy + hooks。**可重构项目 `CLAUDE.md`**。

**lite `/oplinit`**：bind lite 集 + 三区骨架 + profile=lite。低侵入——不加 hook，仅写 `env.OP_DOCS_DIR`。**不修改项目 `CLAUDE.md`**。建 `$OP_DOCS_DIR/e2e/` 作为 lite E2E 默认落点。

skill/agent/脚本通过 `$OP_HOME` 引用资源，使用方项目数据走 `$CLAUDE_PROJECT_DIR`（Claude 内置）。

```
全局 skill（2，install 装到 ~/.claude/skills/）
  opinit      heavy 初始化：bind 项目 skill + 骨架 + hooks
  oplinit     lite 初始化：bind 项目 skill + 骨架

项目 skill（init 时 bind 到 .claude/skills/，软链 $OP_HOME/skills/*）
  heavy 集: opinit opintake oprun opstatus optriage opspec opred
  lite  集: oplinit oplintake oplrun opstatus optriage opspec opred
  对外：opintake/oprun（或 lite 对称）+ opstatus + optriage
  内部：opspec（user-invocable:false）/ opred（user-invocable:false）

agents（4，**只放 $OP_HOME/agents/**，不注册任何 agents/ 发现路径）
  派发：Read 模板 → subagent_type: "general-purpose" + prompt=模板+brief
  op-implementer / op-reviewer / op-evaluator / op-closer（closer 仅 heavy）
...
**lite 的定位**：degraded mode，不是 heavy 同等安全版——无 hook、无 worktree 隔离、无 baseline 对照、evaluator 裸评（**evaluator 验收独立性显著弱于 heavy，详见 §5.7**）。用它换低侵入。

### 5.1 两版差异面（收敛后 6 点）

| 维度 | heavy | lite |
|---|---|---|
| 项目 init | `/opinit`：bind heavy skill + 三区 + profile=heavy + hook + 归档 + 可重构 CLAUDE.md | `/oplinit`：bind lite skill + 三区 + profile=lite；不加 hook、不改已有文档；写 `env.OP_DOCS_DIR` + 项目 `.claude/skills/` |
...
| 脚本/环境入口定位 | `$OP_HOME/scripts/` | `$OP_HOME/scripts/`（两版统一；不装 `~/.claude/scripts/omni_powers/`） |
...
lite「低侵入」= 不加项目 hook、不改用户已有文档与代码结构；仅写项目 `.claude/settings.json.env.OP_DOCS_DIR` 并创建 OP 资产（无 task 分支 / merge gate / hook 注册）。低侵入范畴：

**允许**：
- `~/.claude/` 全局安装（`install.sh --set-ophome`）：仅 opinit/oplinit 软链 + OP_HOME env——用户级配置，不算项目侵入
- 项目 `.claude/settings.json` **仅** `env.OP_DOCS_DIR`（不注册 hook）
- 项目 `.claude/skills/`：`op_bind_project_skills.sh` 软链业务 skill（覆盖前校验 OP 所有权；profile 冲突零写入）
- 项目内 `$OP_DOCS_DIR/`（含 lite `e2e/`）：默认独立子目录；共享 `docs` 时 managed block 不覆盖宿主

**禁止**：
- 注册项目 hook / 改用户已有非 OP 文档与代码结构
- 覆盖非 OP 同名 skill/agent
- 创建 task 分支、merge gate 等 heavy 拓扑

### 5.4 环境入口（两版共用一份 agent 文件）

两版统一用 `$OP_HOME/scripts/`——lite 也需 `install.sh --set-ophome`（写入 `~/.claude/settings.json` 全局 env，不算项目侵入）。agent 内 `op_script()` 单路径直查：

```bash
# agent 内 op_script() 单路径 resolver（脚本统一在 $OP_HOME/scripts/）：
#   [ -n "${OP_HOME:-}" ] && [ -d "$OP_HOME/scripts" ] || { echo "FATAL: OP_HOME 未设或 scripts/ 不存在"; exit 1; }
#   op_script() { ls "$OP_HOME/scripts/$1" 2>/dev/null | head -1; }
#   [ -n "$(op_script op_check_env.sh)" ] || { echo "FATAL: $1 不在 OP_HOME/scripts/ 下"; exit 1; }
```

- **两版统一**：`OP_HOME` 指向仓库根，`OP_HOME/scripts/` 为脚本根。lite 也需 `--set-ophome`。
- **前置探活**：agent 在 resolver 后立即校验 `$OP_HOME/scripts/` 存在，不存在 → FATAL 并停在首个脚本调用前。
- agent 派发机制两版相同：`Read $OP_HOME/agents/op-*.md` → `subagent_type: "general-purpose"` + 模板注入 prompt。**agent markdown 是静态模板，两版共用一份**；不注册为 Claude Code 自定义 subagent 类型。

### 5.5 lite 脚本定位

lite 与 heavy 统一用 `$OP_HOME/scripts/`——无需独立共享目录，无需 skill 内脚本副本。lite 也需 `install.sh --set-ophome`（全局 `~/.claude/` 不算项目侵入；项目级仅持久化 `env.OP_DOCS_DIR`）。

**脚本 profile 入口约定**：heavy/lite 两版共用的脚本必须在入口校验 `OP_PROFILE` 存在且为已知值（heavy|lite），未知值 `die` 而非静默按 heavy 路径执行。

**lite 脚本差异收敛在脚本内 profile 分支**（单份脚本内 `OP_PROFILE=lite` 判定）：
...
拆 task 写 op_execution/tasks_list.json（status=awaiting_gate，depends_on 机读）
【闸门 A】呈报全部 task spec + 拆分给用户审（预算同 heavy：15-30 分钟/需求）
   人批 → spec status: approved + task awaiting_gate→ready（无 git 写保护 hook；靠约定 + git diff 可回溯）
