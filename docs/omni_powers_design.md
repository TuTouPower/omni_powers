...
├── tests/e2e/                        # 【代码，永久资产】heavy E2E 落点（默认 `tests/e2e/`，init 时问用户确认）；lite 默认 `docs/omni_powers/e2e/`（§5.3）
...
**lite `/oplinit`**：三区骨架 + profile=lite。零侵入——不加 hook、不改项目 `.claude/settings.json`、不改造项目文件结构。**`CLAUDE.md` 只允许末尾追加说明段**（告知 `docs/omni_powers/` 目录用途 + e2e 落点 + `/oplintake`/`/oplrun`/`/opstatus` 入口），不删不改原有内容；用户拒绝则跳过。建 `docs/omni_powers/e2e/` 作为 E2E 默认落点。
...
lite「零侵入」= **不侵入用户项目**：不加项目 hook + 不改项目级 `.claude/` 配置 + 不改用户项目已有文件结构（无 task 分支 / merge gate / hook 注册）。零侵入范畴：

**允许**：
- `~/.claude/` 全局安装（`install.sh --set-ophome`）：skill/agent/OP_HOME env——用户级配置，不算项目侵入
- 项目内 `docs/omni_powers/`（含 `e2e/` 验收资产，默认 `docs/omni_powers/e2e/`）：新增独立子目录，不改用户已有文档
...
lite 与 heavy 统一用 `$OP_HOME/scripts/`——无需独立共享目录，无需 skill 内脚本副本。lite 也需 `install.sh --set-ophome`（全局 `~/.claude/` 不算侵入，零侵入指不修改项目级 `.claude/` 配置与文件结构）。
