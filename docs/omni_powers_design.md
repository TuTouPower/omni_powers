# Omni Powers Harness: Claude Code 全流程开发方案设计
...
6. **测试按耦合物分层，按保护物设防。** 行为层（E2E+回归）是“行为不变”的法官，永久锁定（归 evaluator；implementer 经 **worktree 对称隔离**无法触碰——implementer worktree 不挂 `e2e/`，BUG-* 由 evaluator/leader 转交，§2/§10——hook 对 subagent 拦截失效见 §8.1 前提）；结构层（单测）随代码结构机械适配，断言是红线。只保证有一层防篡改且足以拦住 bug。
...
│   ├── op_blueprint/
│   │   ├── prd.md                   # 产品级需求纪要（grill-me 产出，grill-me 由外部 repo 提供；各需求总意图）
...
[Stage 2] task 拆分（opintake 内自动完成，§6）
 ...
 │         ──► Stage 2 执行图自检（自动：扫执行图 + 拆 task 自检——跨 task 决策遗漏则回补 spec 再过 A，可跳过）══► opintake 终点："就绪"状态
 ▼
...
feature: <功能名>    # 对应 op_blueprint/specs/{功能名}.md；closer 合入 baseline 时按此映射（临时区按前缀→合入区按功能名，§8.2）
---
# <名称>
## 一句话意图
...
## 执行图（拆分后由 opintake 追加：task 依赖概览 + 接口 task 位置，≤10 行，Stage 2 自检扫这里）
```
...
写 spec 前先输出假设清单一并供审；审批即 commit + 写保护；闸门 A 焦点：不变量覆盖沉默失败区（数据隔离/持久化/权限）、边界含竞态与失败路径、Then 全部可翻译为断言、技术决策无遗漏、**可测性契约完整**（测试缝全覆盖、预期失败模式每 AC 至少 1 条）。（"拆 task 发现跨 task 决策遗漏"不在此——拆 task 在 Stage 2，归 Stage 2 自检（自动）检查，见 §4。）
...
| plan 信息 | 归宿 | 消费者 |
|---|---|---|
| 顺序与依赖 | tasks_list.json（机读）+ spec 末尾执行图（人扫） | leader（Stage 2 自检自动扫） |
...
1. **结构隔离层（唯一硬底线，worktree 无 src）**：evaluator 在独立 worktree 工作，文件系统只挂载 spec + 生效规格（开工前基线）+ baselines 索引 + 应用启动方式 + 构建产物（Electron 可执行文件 / web dist / 扩展 .zip / 服务二进制）+ `e2e/`。源码 `src/**`、task 目录（`op_execution/tasks/**` + `op_record/tasks/**`）、`op_record/decisions.md` 物理不挂载——结构上不可能读，纪律禁止次要。implementer 分支跑 CI 构建产出打包好的应用供 evaluator 操作。
...
- **写权**：`e2e/`（固化 PASS 测试）+ `op_execution/acceptance/{前缀}/`（baseline 快照 + 验收报告）；其余禁止写（尤其 `op_blueprint/`——per-leaf 由 leader 基于 closer 提案写）——同样靠 worktree 挂载范围 + 写路径限定。
...
防"偷偷改绿"按强度排序。⚠️ **前提**：PreToolUse deny 对 subagent 整体失效（§8.1 前提，依据见 D18），implementer/evaluator 均为 subagent——故以下防线对 subagent 场景不依赖 hook 拦截，主防线靠**结构 + git 层**；hook 拦截仅主会话 leader 场景有效，作 advisory。第 1-5 层是 hook/结构防线，第 6 层是独立 CI 任务（非 Claude Code hook）。

1. **结构隔离 + 行为层保护**（worktree 对称 + git 层，硬底线）：`e2e/**` 对 implementer 通过 **worktree 对称隔离**——implementer worktree 不挂 `e2e/`（§2）；`BUG-*` 新增属 fix 流程（带归因+解锁审批，§1.2），由 evaluator 验收时写或 implementer 产 patch 由 leader 转交，implementer 不直接落盘 `e2e/`；既有 `e2e/`+`BUG-*` 修改硬锁靠 **git 层**（branch protection + pre-commit/post-receive hook 在 git 操作层，非 Claude 工具层——绕过 §8.1 的 subagent deny 失效）。归因记 decisions.md。
2. **访问隔离**（结构性，详见 §8.1）：evaluator 仅接触 spec + 构建产物 + baselines，不接触源码或 implementer 交接文件——靠 worktree 无 src 实现（非 hook）。
3. **机器证据**（PostToolUse + SubagentStop）：改 `src/**` 自动跑受影响测试留原始输出；**SubagentStop**（非主会话 Stop——拦 implementer subagent 交工，matcher 按 agent_type 过滤）检查 tasks_list.json 状态 + 新鲜测试输出，缺则拒收工；hook 脚本开头**必检查 stdin 的 `stop_hook_active` 字段**，为 true 时 exit 0 防递归无限循环；禁 --no-verify。结构层单测明确不设防，由行为层兜住。
...
**防线层 ↔ 实现手段映射**（⚠️ 读者注意：主防线 1/2 层**不在** §11 hook 清单里——配齐 hook ≠ 安全，hook 对 subagent 失效，主防线靠结构与 git）：

| §10 防线层 | 实现手段 | §11 Claude hook？ |
|---|---|---|
| 1 结构隔离 + 行为层保护 | worktree 对称（无 e2e/）+ git branch protection / pre-commit / post-receive | ❌ 非 hook（结构 + git） |
| 2 访问隔离 | evaluator worktree 无 src（§8.1） | ❌ 非 hook（结构） |
| 3 机器证据 | PostToolUse[src/**] + SubagentStop | ✅ hook |
...
## 11. 插件结构（skill 安装 + `$OP_HOME` 环境变量）

> **安装模型**（非 Claude Code plugin 市场机制）：用户 git clone 本仓库 → 跑 `/opinit` skill 安装 → opinit 引导把 `$OP_HOME`（=插件安装目录）写入使用方 `.claude/settings.json` 的 `env` 段。skill/agent/hook/脚本通过 `$OP_HOME` 引用插件资源（RULES/design/scripts/docs_template/hooks），使用方项目数据走 `$CLAUDE_PROJECT_DIR`（Claude 内置）。废弃 `$CLAUDE_PLUGIN_ROOT` / plugin.json / hooks.json / `claude plugins install` 机制。用户可在环境变量里自定义 `$OP_HOME` 及模型档位等配置。
...
hooks（⚠️ 前提：PreToolUse/PostToolUse deny 对 subagent 整体失效，依据见 D18——下列 hook 仅主会话 leader 场景作 advisory 生效；subagent 隔离靠 worktree 结构 §8.1/§10）
  PreToolUse[Edit/Write]   主会话守门（subagent deny 失效）：spec 写保护/op_blueprint 写拦截（仅 leader 审批脚本如 op_merge_blueprint.sh 可写）/行级敏感度。e2e/**+BUG-* 主会话 advisory；subagent 靠 worktree 对称 + git 层（§10）
  PreToolUse[Task]         dispatch 协议 advisory 留痕：fire 能读 dispatch prompt 记日志，deny 拦不住 launch（依据见 D18）；不作拦截主张
  PostToolUse[src/**]      主会话场景自动跑受影响测试留证据；subagent 场景不触发，靠 SubagentStop 兜
...
**P0（第一周，零基建）**：opinit 骨架；入口分拣两判定 + spec 模板与命名约定（含可测性契约）+ 闸门 A + 审批即 commit；任务卡 + task=commit；红灯归因、可写性矩阵、review 两轮上限、契约边界规则进 RULES.md/test.md；完成必须贴测试输出；issues 手工登记。**worktree 无 src 工程 spike 启动**（evaluator/implementer worktree 挂载范围 + CI 构建产物生产链路可行性，§8.1 前提，硬底线必须结构隔离）。
...
**P2（第三-四周，subagent 层）**：先落 **reviewer 双裁决 + closer 两段节奏**（per-task append decisions / per-leaf 验收后提案）+ 循环上限（review ≤2 轮、Stage 4 验收 ≤3 轮）+ issues 自动登记 → 再落 **op-evaluator 浏览器基建作为自举第一 spec**（Electron/扩展驱动是最高技术风险，顺便用流程验证流程）。⚠️ **自举例外**：evaluator Stage 4 能力正是这条 spec 要造的东西，引导期首 spec 走**人工/降级验收**（leader 对照 spec + 破坏检查手工做），evaluator 上线后才转标准 Stage 4。→ evaluator Stage 4（评估→固化→破坏检查）→ e2e/ 夜跑 → 叶子归档流。**evaluator worktree 无 src 从 P2 起硬要求**（hook 对 subagent 失效，纯靠结构隔离，依据见 D18）+ **防放水前三层（hard-pass gate + 预期失败模式 + 破坏检查）prompt/spec/脚本内置，P2 全上**（几乎免费）。

**P3（持续）**：生效规格与 domain.md 沉淀；变异测试体检；issues triage 节奏；模型升级后审视护栏做减法。**独立验证环境增强**（CI 构建产物链路优化 + 独立机器自由操作 UI——P2 worktree 无 src 已是基线，P3 做体验/可靠性增强，非从零交付）。**防放水后两层（baseline 对照评 + 刻薄化调教循环/钓鱼审计）等前三层上线后观察到真实放水案例再加**——基准维护与二阶判断是持续运营成本，未验证假设前不预付。注意：**P2 已建 baseline 快照**（evaluator 固化 PASS 时存），P3 才做系统化对照评/钓鱼调教，不是 baseline 本身 P3 才有。
