...
[Stage 3] 逐 task 循环（§7）：brief → op-implementer → hook 证据
 ...
 │         破契约：spec 变更子流程（人批）
 │         （技术探针验证：风险探针 task 完成后 leader 跑通证伪路线，≤10 分钟，非正式验收、不派 evaluator）
 ▼
...
例外：**技术探针验证**（风险探针 task，每 spec 至多一两个）完成后 leader 跑通证伪路线，≤10 分钟——**非正式验收、不派 evaluator**（evaluator 仅 Stage 4）。与方案先行一前一后，分别夹住"想错了"和"环境不配合"。
...
> ⚠️ **共同前提风险**：三层全依赖 hook 能识别当前 agent 身份（implementer/reviewer/evaluator）——所有按身份分级的拦截（Read/Grep/Bash/dispatch）都建立在此。agent 身份识别 + Task prompt 字段拦截，落地待 P2 验证可行性；约束先立。
...
1. **物理锁**（PreToolUse）：`e2e/**` 对 implementer 全局硬阻断（仅 evaluator 可写，§2）；`BUG-*` **新增对 implementer 放行**（fix 先红后绿必需，带 fix 归因 + 解锁审批）、**修改既有硬阻断**（改既有属归因(b)，须写明依据 + 人工解锁）。归因记 decisions.md。
...
**P3（持续）**：生效规格与 domain.md 沉淀；变异测试体检；issues triage 节奏；模型升级后审视护栏做减法。**防放水后两层（baseline 对照评 + 刻薄化调教循环/钓鱼审计）等前三层上线后观察到真实放水案例再加**——基准维护与二阶判断是持续运营成本，未验证假设前不预付。注意：**P2 已建 baseline 快照**（evaluator 固化 PASS 时存），P3 才做系统化对照评/钓鱼调教，不是 baseline 本身 P3 才有。
