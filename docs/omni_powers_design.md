...
│       └── tasks/                   # 已完成 task 的 brief/report/review 归档（blueprint_update.md 随 acceptance 工作区归档）
```
...
Stage 2（验收先行）已删除。evaluator 仅在全 task 闭环后的 Stage 4 介入一次。"可测性契约"作为 spec 模板必填小节（§5.1），闸门 A 一并过审——零新增 agent、零独立 token 消耗。原来的"时序隔离 + 提前锁定"替换为"**访问隔离 + 破坏检查**"。
...
**per-task 收口（轻，task 闭环即做）**：

1. `scripts/op_close_pre.sh {TID}`：spec 盖戳 + `status=收口中`。
2. op-closer 直接 append 决策到 `op_record/decisions.md`（append-only，不经 leader 审批）。
3. `scripts/op_close_post.sh {TID}`：确认 review verdict PASS、spec 已盖戳，git mv 归档 task 目录到 `op_record/tasks/{TID}/`、追加 progress、`status=完成`。
   - **铁律**：per-task 阶段 closer 不产 blueprint 提案、不碰 op_blueprint、不碰 git、不改 status、不 stage。

**per-leaf 收尾（重，Stage 4 验收 PASS 后做一次）**：

1. 所有 task 闭环 → Stage 4 验收 PASS → leader 派 op-closer 做叶子收尾。
2. op-closer 产「blueprint 更新提案」写入 `op_execution/acceptance/{前缀}/blueprint_update.md`，diff 形态覆盖 `op_blueprint/` 全部文档（specs/{feature}.md、architecture.md、domain.md、conventions.md、baselines/ 等）：这条新增、这条修改、这条因被上游覆盖而删除，各附一句理由。只留"现在是什么"，过滤被否方案/临时假设。**含 baselines 合入段**（新增/更新/删除各附 AC 与理由）+ **叶子归档提案**（总述关闭、前缀释放）。**吸收验收结果**：实现中发现的未预见边界行为、FAIL 修复后的最终形态一并写入。
   - **铁律**：op-closer 对 `op_blueprint/` 无写权限；不碰 git、不改 status、不归档、不盖戳、不 stage。
3. leader 审批 closer 的收尾提案 → 执行实际写入 `op_blueprint/`（specs + baselines 合入）→ 叶子归档（spec 原文入 `op_record/specs/`、acceptance 工作区归档）→ commit。
...
1. **文件系统层**：hook 硬拦 evaluator 的 Read/Grep 命中 `src/**` + `op_execution/tasks/**`（report/review/diff 是 implementer 交接文件；脚本组装的 brief 只管 brief 内容，管不住 evaluator 主动 Read，必须 hook 补拦——否则白名单白费。机械可审计，零基建）。
...
4. **刻薄化调教循环**（持续）：
   ...
   - **收敛标准（钓鱼审计，独立于上游质量）**：leader 定期钓鱼——在构建产物里植入一个已知 bug（或悄悄关掉某条 AC 的功能开关），看 evaluator 抓不抓得到。测的是判别力本身，不随 implementer 质量起伏（上游越烂 evaluator 越容易找到独有 bug，不能拿它当收敛判据）。连续 3 次钓鱼全中 → 降频抽查（每 5 spec 一次）。漏钓 → 每次全查并补 few-shot 校准。
...
- **跨叶子 / 后续迭代重验**（前叶子已收尾合入）：读 `op_blueprint/baselines/index.md` 找已有基准快照。
...
**跨前缀更新**：b02 的实现可能合法改变 a 前缀的页面布局，使 a 的旧基准"不一致"。b02 验收时 evaluator 标记 a 基准为 NEEDS_UPDATE，b02 的 closer 收尾提案里附 a 基准更新段（注明被哪个 b02 AC 触发），leader 审批后更新 a 前缀基准。规则：跨前缀更新必须经 closer 提案 + leader 审批，evaluator 不直接改 op_blueprint。
...
2. **访问隔离**（结构性）：evaluator 仅接触 spec + 应用产物，不接触源码或 implementer 交接文件；文件系统层（hook 拦 `src/**`+`op_execution/tasks/**`）+ 报告回流层（`op_assemble_eval_brief.sh` 脚本机械组装）+ dispatch 协议层（模板 prompt + hook 审计）。见 §8.1。
...
hooks（7）
  ...
  PreToolUse[Read/Grep]    evaluator 访问隔离硬防线：拦 evaluator 命中 src/** + op_execution/tasks/**（report/review/diff；brief 白名单管不住主动 Read）
...
scripts/（确定性计算全归 bash，不留给模型）
  工作集 tokenize 核算 / review-package 生成 / eval brief 机械组装（op_assemble_eval_brief.sh） / tasks_list 读写 / checkpoint 读写
```
...
**P0（第一周，零基建）**：opinit 骨架；入口分拣两判定 + spec 模板与命名约定（含可测性契约）+ 闸门 A + 审批即 commit；任务卡 + task=commit；红灯归因、可写性矩阵、review 两轮上限、契约边界规则进 RULES.md/test.md；完成必须贴测试输出；issues 手工登记。
...
**P2（第三-四周，subagent 层）**：op-evaluator 浏览器基建作为自举第一 spec（Electron/扩展驱动是最高技术风险，顺便用流程验证流程）→ evaluator Stage 4（评估→固化→破坏检查）→ reviewer 双裁决 + closer 两段节奏（per-task append decisions / per-leaf 验收后提案）+ 循环上限 + issues 自动登记 → e2e/ 夜跑 → 叶子归档流。**防放水前三层（hard-pass gate + 预期失败模式 + 破坏检查）prompt/spec/脚本内置，P2 全上**（几乎免费）。

**P3（持续）**：生效规格与 domain.md 沉淀；变异测试体检；issues triage 节奏；模型升级后审视护栏做减法。**防放水后两层（baselines 对比 + 刻薄化调教循环/钓鱼审计）等前三层上线后观察到真实放水案例再加**——基准维护与二阶判断是持续运营成本，未验证假设前不预付。
...
| decisions.md | 写入者未明 | **两写入者**：spec 编写者（设计探索全文，§5.2）+ closer（执行期自决决策，append-only） |
