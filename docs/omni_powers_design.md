...
9. **task 即 commit；粒度沿低耦合缝隙切。** 沿层/模块/数据流阶段的缝隙切；"task 即 commit"由 leader squash-merge 兑现（§3.4）。**task 严格串行执行**——tasks_list 的 depends_on 只表达"无依赖"事实，不授权并行：多 implementer 并行意味着共享流程文件（decisions.md/issues/tasks_list——单一物理副本直写主 repo，§3.4）并发写入 + 多 task 分支同时回流 merge，冲突协议未建前不放开。模型/基建升级后若开并行，先解决共享文件写入协议再放开。
10. **循环必须有上限，发现必须有去处。** review/fix 最多两轮；修不完的和范围外的统一落 issues。
11. **正常路径下，人只出现在两个位置：闸门 A（审 spec，含技术决策与可测性契约）、闸门 C（验收报告 + spec 变更决策表 + P0/P1 issue；lite 无闸门 C，见 §5）。** 异常路径仍有人裁：review 两轮到顶升级、验收三轮到顶升级、阻塞 issue 转 task——这些是异常出口，不是常设闸门。闸门 A 预算 **15-30 分钟/需求**（一次 intake 拆出的全部 task spec 一次呈审）：spec 含不变量(INV)+验收标准(AC)+边界+三类技术决策+可测性契约，是全系统唯一质量单点，5-10 分钟审不完只会橡皮图章——spec 错则三方全错且无人能拦。闸门 C 1-2 分钟，只读自然语言；人不审文件/函数清单——那是杠杆错位。
...
├── e2e/                             # 【代码，永久资产】heavy：顶层 e2e/，按 TID 分目录的验收 E2E 全集 + BUG-* 回归，系统层定期全量跑
...
│   │   │   ├── T001_darkmode.md     #   每 task 一份，自足（共享不变量/跨 task 技术决策复制进每个相关 spec，不引用）
│   │   │   └── T002_pages.md        #   frontmatter: status: draft|approved|in_progress|done
│   │   │                            #   TID 全局单调递增 T001/T002…永不复用（e2e/baselines/归档按 TID 存）
...
**TID 编码**：全局单调递增 `T001/T002/…`，固定三位数宽度，永不复用（e2e/baselines/op_record 永久工件按 TID 存，复用会撞）。一次 intake 拆出的多个 task 连续编号；不同 intake 续编。spec 文件名 `specs/{TID}_{slug}.md`。
...
│   └── review.md              # 双裁决结论（reviewer 无 checkout，结论经返回结果由 leader 落盘，§3.4）；FAIL 轮 implementer 追加 Fix-N（task 目录已挂载）
├── tasks_list.json            # task 元数据唯一源（id/title/status/spec/depends_on/workset）
...
- **无 brief 文件**——dispatch 时 leader 在 prompt 给指针（TID + spec 路径），agent 读 spec（契约）+ `jq` 查 tasks_list.json 取该 task 元数据（workset/depends_on）+ heavy 按需读 blueprint（约定）；lite spec 自足。单一真相，零重复。
...
- **task 级，验收后**（evaluator 验收 PASS → 闸门 C 收尾）：task 的全部验收标准验收 PASS → closer 产 per-task 收尾提案 → leader 审批 → 精华并入生效规格（后续 task 引用它作基准）→ 原文入 op_record/specs/ → tasks_list 清出其 task。进入生效规格的是淬炼后的结论："待澄清"、被证伪的担忧不进；实现中发现的未预见边界行为 + 验收 FAIL 修复后的最终形态一并补进。（lite：无 closer/blueprint，leader 直接归档 spec 原文，§5.6）
...
[Stage 1] 工作 spec 编写（含内联设计探索 + 可测性契约，§2.2）
 │         task:spec = 1:1；共享不变量/跨 task 技术决策复制进每个相关 spec（自足，不引用别的 spec）
 │         ──► 【闸门 A：15-30 分钟/需求——一次 intake 的全部 task spec 一次呈审】──► approved，写保护
 │
...
[Stage 3] 逐 task 循环（§2.4，严格串行）——每 task 走完整链后下一个：
 │   dispatch 指针（TID+spec 路径）→ op-implementer（TDD：先写映射验收标准的失败单测贴 RED → 最小实现 → 绿）→ 测试证据
 │   → op-reviewer 双裁决（≤2 轮）；两轮修不平 → 记 issue + task 标阻塞 + 下一个 task
 │   → squash-merge 回主分支（过 merge gate，§3.4）
 │   → op-evaluator 验收（每 task 一次）：访问隔离 eval_brief（spec+生效规格基线+启动方式+baselines 索引，不含 implementer 产物）
 │     逐条验收标准 hard-pass gate + 固化 PASS 测试 + 破坏检查 + 对抗探索；范围内 FAIL → 修复回流（≤3 轮）；到顶 → 记 issue + task 标阻塞 + 下一个 task；范围外 → issues
 │   执行期决策（§2.4）：小决策（选库/算法/路径，不进 spec）直接做不记录；需进 spec 的 leader 改 task spec + 记 decisions.md（spec-delta）+ 事后报；不阻塞等人
 ▼
 task 验收 PASS ──► [Stage 4] closer 收尾 + 闸门 C（§2.6）
 │   closer 一段式：产 per-task 收尾提案（提取红灯归因 append decisions + blueprint diff + baselines 合入段 + task 归档提案，吸收验收结果）
 │   → 【闸门 C：1-2 分钟】leader 审批 → 写入 op_blueprint + baselines 合入 + task 归档 + commit → 删 task 分支与 worktree → 下一个 task
 │   呈报：验收报告（evaluator 产）+ 验收标准追溯矩阵（closer 提案含）+ spec 变更决策表（decisions.md spec-delta）+ P0/P1 issue（P0 默认阻断，用户可显式豁免并记 decisions）
 │   leader 做 evaluator 二阶判断 → 偏差指令 → 积累校准素材
 │   （lite: 无闸门 C，但保留 P0 阻断检查——oplrun task 归档前 open P0 issue 存在时停下问用户，§5.8）
 ▼
...
type: feat           # feat | fix | refactor | perf | style | test | fix | ...
---
# <名称>
## 一句话意图
...
### 可测性契约（必填）
   ...
   - 预期失败模式（每条验收标准至少 1 条——若 xxx 没做好，验收标准应该 FAIL；evaluator 验收 对照此表逐条试反例）:
...
leader-worker：leader 只编排，上下文只留状态；交接全走 task 目录文件；每 task 全新 subagent 独享完整上下文。**task 严格串行**——tasks_list 的 depends_on 只表达"无依赖"事实，不授权并行（共享流程文件并发写入 + 多分支回流 merge，冲突协议未建前不放开）。

#### 单 task 循环

1. **dispatch op-implementer**（prompt 给指针：TID + spec 路径；agent 读 spec 契约 + `jq` 查 tasks_list.json 取 workset/depends_on + heavy 按需读 blueprint 约定）
   - TDD：先写映射验收标准的失败单测，贴 RED → 最小实现 → 测试证据
   - 证据可信度：implementer 自跑自贴，靠 reviewer + evaluator 兜底
   - 写 report.md（顶部总报告覆盖 + 分 Round 追加）→ 返回 `DONE | BLOCKED | NEEDS_CONTEXT`
2. **dispatch op-reviewer**（只读 review-package：report + 三点 diff + spec）
   - diff 为脚本生成的 `主分支头...task分支` 三点 diff，防挑选性呈现（§3.4）；reviewer 另 `jq` 查 tasks_list 取 workset 对照越界
   - 双裁决：
...
4. **双裁决 PASS** → merge gate 校验 + squash-merge 回主分支（§3.4）
5. **dispatch op-evaluator**（每 task 一次，验收该 task spec 的验收标准）
   - 访问隔离：eval_brief 机械组装（spec + 生效规格开工前基线 + 启动方式 + baselines 索引，不含 implementer 产物），机制详述见 §2.5
   - 逐条验收标准 hard-pass gate + 固化 PASS 测试 + 破坏检查 + 对抗探索
   - 范围内 FAIL → 修复 task 回流（≤3 轮）；到顶 → 记 issue + task 标阻塞 + 下一个 task；范围外 → issues
   - 验收 PASS → 进 Stage 4（closer 收尾 + 闸门 C，§2.6）
...
2. **记录决策**：leader 写 delta（原文引用 + 改后文本 + 理由，逐条）append 到 decisions.md（来源标记 spec-delta）。
3. **改 spec**：leader 解锁 task spec（heavy: git 解锁脚本；lite: 直接改）→ 应用 delta → 重新 commit + 恢复写保护。
4. **task 处置**：受影响 task（spec 的验收标准/不变量与 delta 相交）——未开始的改 tasks_list 该条；进行中/已完成的标 `cancelled` 并重拆（已有 commit 不 revert，append-only，§2.3）。
...
op-evaluator 在 Stage 3 task 循环内介入（每 task 一次）。per-task 验证：每 task merge 后即派 evaluator **自己操作应用**复现该 task spec 的验收标准，逐条验收标准 binary gate（hard-pass gate）+ 固化 PASS 测试 + 破坏检查 + 对抗探索，逐条验收标准报告。范围内 FAIL → 修复 task 回流（≤3 轮）；**到顶处置**：验收标准是 binary gate，不存在"降级落 issue"——到顶即记 issue + task 标阻塞 + 下一个 task。**review 上限 2 轮（reviewer 裁决）与验收回流上限 3 轮（evaluator 发现验收标准不通过后的修复重验）性质不同，独立设定**；范围外发现（不属本 task spec 验收标准的问题/可用性建议）→ issues（§3.2）。PASS → 派 op-closer 一段式收口（§2.6）。

#### evaluator 访问隔离与防放水

stock model 默认对 LLM 产出宽容——能发现 bug 但会说服自己"不太严重"放行，或只测成功路径不探边界。隔离防防水"抄实现"（evaluator 读源码后照着实现写测试→实现错→测试跟着错→一起绿）靠以下机制。
...
2. **预期失败模式**（spec 可测性契约）：每条验收标准附 1 条反例——若 xxx 没做好则 验收标准应该 FAIL。evaluator 验收 逐条试。零 token 零 agent，写 spec 时顺手加。
...
- **重验（对照评）**：evaluator 重新操作应用复现同一验收标准路径，**逐步对照 baseline 记录的"该观察到什么" + spec 目标**判断。截图不是比对对象，是"上次操作到这步看到了啥"的参考锚点；结构化信号才是机械比对的硬证据。结果——一致 / REGRESSION（视觉层 advisory 不阻断）/ 预期改进（更新基准，走提案制）。
...
| 视觉 | 多模态对照，evaluator 自己判 | 不进机械硬门 | 截图——操作到某步"该看到啥"的锚点，由 evaluator 多模态对照（看语义级差异、不被渲染噪声炸；靠 hard-pass gate + 预期失败模式兜） |
...
- **全批** → 执行写入 `op_blueprint/`（specs + baselines 合入）→ task 归档（spec 原文入 `op_record/specs/`、task 目录入 `op_record/tasks/{TID}/`、acceptance 工作区入 `op_record/acceptance/{TID}/`、tasks_list 标完成、progress 追加）→ commit → 删 task 分支与 worktree
- **部分批** → leader 逐条标采纳/修改/驳回 + 批注，closer 按批注修订提案重提（**上限 2 轮修订**；到顶仍不达标则 leader 亲自改定提案文本后执行写入，或整体驳回）
- **驳回** → 回 evaluator 重验收，或 closer 重写提案

**呈报四样**（各有主）：验收报告（evaluator 产，§2.5）+ 验收标准追溯矩阵（closer 提案含）+ spec 变更决策表（从 decisions.md 提取 spec-delta 标记）+ P0/P1 issue（脚本从 issues/ 提；P0 默认阻断，用户可显式豁免并记 decisions）。人不审文件/函数清单——那是杠杆错位。

#### baselines 合入流程

closer 在 blueprint_update.md 的 baselines 段列出新增/更新/删除（各附验收标准与理由），leader 审批后：

- 新基准从 `acceptance/{TID}/baselines/`（**临时区，按 TID**）合入 `op_blueprint/baselines/{功能名}/`（**合入区，按功能名**——功能名由 closer 从 task spec 内容判断写进提案，§2.6；不进 frontmatter，非机械字段）
- 更新 `op_blueprint/baselines/baselines_index.md`（追加/修改对应行）
- 删除的基准从 op_blueprint 移除

**跨功能更新**：一个 task（如 T03_contact）的实现可能合法改变**另一个功能**（如 darkmode）的页面布局，使该功能旧基准"不一致"。验收时 evaluator 标记该功能基准为 NEEDS_UPDATE，closer 收尾提案里附该功能基准更新段（注明被哪个验收标准 触发），leader 审批后更新该功能基准。规则：跨功能更新必须经 closer 提案 + leader 审批，evaluator 不直接改 op_blueprint；**只动 `op_blueprint/baselines/{功能名}/`**——若该功能的语义契约本身变了（验收标准/不变量改），另开该功能的 spec 变更子流程，不混在 baseline 更新里；**既有 e2e 同规则**：需改另一功能既有 e2e 时，走 closer 提案 + leader 审批（§3.1 改既有 e2e = spec 变更），evaluator 不直接改。
...
**P 级（P0-P3）是 issue 排期语义**，落 issue 时赋值。review 不按严重度分流——范围内问题修（fix 循环），范围外问题落 issue。落盘者赋 P：reviewer 范围外发现/暂存项写 issue 时定 P（最了解影响面）；evaluator/夜跑/体检同理。optriage 每 task 收尾复核可升降级（分诊需全局视野，是 P 级最终裁定者）。P0 只能由人或 optriage 复核确认（阻断语义重，不许 agent 单方面赋 P0 后静默放行——赋了就会触发阻断检查，语义自洽）。

```markdown
---
id: I-20260702-01
title: 会话列表 200+ 会话滚动掉帧
source: evaluator 范围外（T03 验收）    # review 两轮到顶 / reviewer 范围外 / evaluator 范围外 / 系统层夜跑 / 定期体检
spec: T03
...
铁律：**issue 不直接改代码，转正式 task 后走对应 change type 流程**（fix 带回归测试先红后绿）——issues 是登记处不是免检通道。每 task 收尾时 optriage 一次；**P0 默认阻断**：heavy 阻断闸门 C（§2.6）；lite 无闸门 C 但保留同语义检查——oplrun task 归档前扫 issues/，存在 open P0 则停下问用户（豁免记 decisions，§5.8）。P1 默认进入下个 task 前必修；若用户显式豁免，必须记录 decisions。
...
4. **spec 写保护**（merge gate 为主，git 原生辅助）：approved/in_progress 的 spec 路径变更被 merge gate REJECT（task 分支侧，硬）；主分支侧 git pre-commit 拦 leader 误改 + 主会话 PreToolUse 拦截（走 §2.4 变更子流程）；SessionStart 注入前 git diff --quiet 校验，防"好心更新规格"漂移。
...
| op-evaluator | 独立 worktree | sparse：排除 `src/**`+task 目录+decisions.md | `op/eval/{TID}` |
| op-reviewer | 只读，不需要 checkout | 读 leader 提供的 review-package（diff + report + spec，脚本生成）；review 结论在返回结果中给出，**由 leader 落盘 `tasks/{TID}/review.md`**（FAIL 轮 implementer 追加 Fix-N——task 目录已挂其 worktree） | — |
| op-closer | 主 worktree 内工作（heavy） | 完整（只写 decisions.md/issues/，权限清单 §2.4） | 主分支（写入经 leader commit） |

流程文件（tasks_list.json / checkpoint / issues / decisions.md）**只在主 worktree 一份物理副本**，implementer/evaluator worktree 不挂 `op_execution/`+`op_record/`，例外仅三处：task 目录挂给 implementer 写 report；`op_execution/specs/{TID}_{slug}.md` 只读挂给 implementer（dispatch 指针指向 spec 路径，§1.1，不挂则无文件可读；merge gate 拦其变更入主分支，只读挂载不破坏写保护）；`op_execution/acceptance/{TID}/` 挂给 evaluator（其唯一可写流程目录，见下）——消除"worktree 副本独立导致下一个 agent 看不到写入"问题的根源。
...
4. **merge gate（机械校验，`op_merge_gate.sh`，PASS 才许合）**：
   - 受保护路径零 diff：`op_execution/specs/**`（approved spec）、`e2e/**`（含 BUG-* 回归，§3.1 钉死其归属）、`op_blueprint/**`、`op_record/decisions.md`、tasks_list.json——task 分支对这些路径的任何变更直接 REJECT（合法变更走各自专属通道：spec 变更子流程、e2e leader 入口、closer 提案）。
   - review verdict PASS 存在（读 review.md 末行）。
   - 工作集越界检查：实际 diff 文件集对照 tasks_list 的 workset，超限额外列出（advisory，供 leader 判断范围偏航）。
5. **squash-merge**：leader `git merge --squash op/task/{TID}` → 单 commit 进主分支（兑现 task 即 commit）；commit message 引用 TID+验收标准。**冲突处置**：串行执行下主分支自 task 切出后仅有 leader 的流程文件 commit，src 冲突理论不发生；一旦发生（说明切出后主分支动过 src）→ 停下按 spec 变更子流程或重切 task 处理，**leader 不代 implementer 解决 src 冲突**（避免 leader 写实现代码，越权且上下文污染）。
6. **收尾**：merge 后进 evaluator 验收（§2.4 循环内）→ closer 一段式收尾 + 闸门 C 归档（§2.6，此时删 task 分支与 worktree）。

**evaluator 产物回流**：固化 PASS 测试在 `op/eval/{TID}` 分支的 e2e/ 下；leader 审后 cherry-pick/merge 进主分支（P2 起过 trailer 配对入口，§2.5——merge gate 对 e2e 的 REJECT 仅针对 task 分支，eval 分支走 leader 专属通道）。验收报告/baselines 写 `op_execution/acceptance/{TID}/`（挂载给 evaluator 的唯一可写流程目录）。
...
agents（4，装 ~/.claude/agents/，两版共用文件——环境入口 profile 化 fallback，§5.4）
  ...
  op-closer         heavy 独有（lite 不派发）。一段式（per-task，验收 PASS 后）：产 blueprint_update.md
                    提案（diff 覆盖 op_blueprint 全部文档 + baselines 合入段 + task 归档，吸收验收结果）+ append decisions.md + 转暂存 issue；对 op_blueprint 无写权
                    对 op_blueprint 无写权
...
**用户旅程**：`install.sh` 一次（全局）→ 每项目 `/opinit` 或 `/oplinit` 一次 → 每需求 `opintake`/`oplintake "..."` → 批 spec（闸门 A）→ `oprun`/`oplrun` → 中途 `opstatus` → heavy 闸门 C 批"验收报告 + spec 变更决策表 + P0/P1 issue"；lite 自动完结（P0 检查兜底）。两个命令干活，一个看状态，heavy 人工两次点头 / lite 一次。
...
循环（每 task，选 depends_on 全完成、ID 最小，严格串行）：
  ...
  派 op-reviewer → 双裁决 → tasks/{TID}/review.md（末行 verdict）
    ...
    └─ PASS → leader 收口（git add workset + commit，§5.9）
      → evaluator per-task 裸评：派 op-evaluator（裸评退化，§5.7）→ E2E + 验收标准逐条 + 破坏检查 + 对抗探索
        ├─ FAIL(≤3轮) → 修复 task 回流重验（到顶异常人裁）
        └─ PASS → 【P0 阻断检查，§5.8】→ leader 归档 task（无闸门 C）→ 下一个 task
全部 task 闭环 → 完结报告
```
...
- **能做**：逐条验收标准评估、跑/写 E2E、破坏检查、对抗探索（首次评）。⚠️ **独立性仅靠角色提示词维持，无法防止无意识实现耦合**（E2E 期望可能与实现同源偏航、破坏检查因读了实现而失独立）——退化程度比"裸评"字面更重。
...
**opspec profile 参数**：heavy 可引用 op_blueprint/specs 映射；lite 只生成 op_execution/specs/{TID}_{slug}.md 单份，不要求 blueprint 映射。功能归属（op_blueprint/specs 按功能划分）不进 frontmatter，由 closer/leader 判断（§2.6）。
...
- oplrun 在 evaluator 验收 PASS 后、task 归档前，扫 `op_execution/issues/`：存在 `severity: P0` 且 `status: open` 的 issue → **停下呈报用户**（issue 清单 + 阻断原因），用户三选一：转修复 task 回流 / 显式豁免（记 decisions，来源标记 leader-close）/ 中止归档。
