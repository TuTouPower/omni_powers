...
三条全不中（改样式、加索引、改变量名、三行 fix）→ 不写 spec 不进 task 机制，契约就是任务卡一句话或那条回归测试。**轻量直做门禁**：按 change type 测试规则（§2）+ 机器证据（贴测试输出）+ commit，不派 reviewer/evaluator、不走 task 循环。
...
1. leader 跑 `scripts/op_close_pre.sh {TID}`：spec 盖戳 + `status=收口中`。
2. **op-closer 只做这步**：append 决策到 `op_record/decisions.md`（append-only，不经 leader 审批）。
3. leader 跑 `scripts/op_close_post.sh {TID}`：确认 review verdict PASS、spec 已盖戳，git mv 归档 task 目录到 `op_record/tasks/{TID}/`、追加 progress、`status=完成`。
   - **铁律**：per-task 阶段 closer **只写 decisions.md**——不跑脚本、不碰 git、不改 status、不 stage、不产 blueprint 提案。脚本（盖戳/git mv/status/progress）全归 leader。
