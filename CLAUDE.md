...
`bash uninstall.sh` 反向 `install.sh`：删 `~/.claude/` 下全局 op\* skill/agent，移除 `settings.json` 的 `env.OP_HOME`（改前自动备份）。`--purge-project` 额外清理当前项目的 `docs/omni_powers/` 与已注册的 hook；`--dry-run` 预览，`-y` 跳过确认。
