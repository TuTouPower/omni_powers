...
bash install.sh --set-ophome   # 全量装 skill+agent 进 ~/.claude/ + 写 OP_HOME（两版都需要）
```
...
/opintake    # 需求入口：spec 编写（含设计探索，task:spec 1:1）→ 闸门 A → 自动拆 task → task 待开始
/oprun       # 从 checkpoint 续跑：task 循环（review → per-task 验收（merge 前验）→ merge → closer 收尾 → 归档）
...
/oplrun                     # task 循环（implementer → leader 自验 → reviewer → per-task 裸评（evaluator）→ leader 收口（commit+归档）→ P0 检查）
```
...
├── scripts/                  # 公用脚本（两版统一入口，$OP_HOME/scripts/）
│
├── docs/                    # 项目文档
...
`bash uninstall.sh` 反向 `install.sh`：删 `~/.claude/` 下全局 op\* skill/agent，移除 `settings.json` 的 `env.OP_HOME`（改前自动备份）。`--purge-project` 按项目 `env.OP_DOCS_DIR` 清理 OP 资产与已注册 hook；共享 `docs` 根保留宿主内容；`--dry-run` 预览，`-y` 跳过确认。
