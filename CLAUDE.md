...
支持两模式：**heavy**（全量，hook 强制 + worktree 隔离 + blueprint）与 **lite**（零侵入，不加 hook / 不改用户配置与已有文档）。设计见 `docs/omni_powers_lite_design.md`。
...
│   ├── omni_powers_design.md #  设计档案（heavy）
│   ├── omni_powers_lite_design.md # lite 设计与两版共存架构
...
| lite 设计与两版共存架构 | `docs/omni_powers_lite_design.md` |
