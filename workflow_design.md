...
| FAIL 轮小修（可选） | `task_review.js`（autofix 模式） | 仅 scope 内 lint/断言/边界/类型小修，1 轮 |
...
| 脚本 | 定位 | 状态 |
|---|---|---|
| `task_review.js` | **主用**。单 task review gate：并行双 review + schema verdict + 可选 autofix 模式 | 待跑通 |
...
`task_review.js` 的 autofix 模式（传 `autofix.scopeFiles`），只处理局部小修：lint、测试断言、小边界条件、类型错误。**写死的限制**：
...
3. `task_review.js` 稳了再上 `task_full.js`
