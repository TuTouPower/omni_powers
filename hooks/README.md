...
| post-receive | 本地 CI 等价物触发——bare 仓库接收 push 时跑 `scripts/op_ci_local.sh`（per-task 证据脱离被监督者控制）；单仓库可用 post-commit 或 leader 手动跑 |

## CI 链路（design §3.3.1 三接口，本地等价物）

`scripts/op_ci_local.sh` 实现本地 CI（无远程 CI 项目用），三接口结果落 `.ci-results/{sha}/`：

| 接口 | 环境变量 | 产出 |
|---|---|---|
| ① 跑测试 | `OP_TEST_CMD` | `test.log` + `test.exit` |
| ② 只读跑 e2e | `OP_E2E_CMD` | `e2e.log` + `e2e.exit`（不写代码，回传结果） |
| ③ 构建产物 | `OP_BUILD_CMD` + `OP_BUILD_OUT` | `build.log` + `artifacts.tar.gz`（evaluator 取） |

`result.json` 汇总三接口 exit 状态，reviewer/leader 读裁决（per-task 证据脱离 implementer 控制）。安全：post-receive 服务端跑，agent 不在进程；结果带 commit-sha；`.ci-results/` 应在 worktree 排除（防 agent 篡改）。
