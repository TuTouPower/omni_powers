...
> - **明确失能 + 提示词级隔离（A11）**：lite 无文件系统隔离，evaluator 物理能读 src/ 与 task 目录——**但 lite 下硬约束：禁止主动 Read `src/**` 与 task 目录的实现细节，E2E 期望只能从 spec 推导**（lite dispatch prompt + 本分支强制，是 lite 唯一防线，弱于 heavy 结构隔离，无法机械拦截有意规避）。无 baseline 对照、无跨迭代回归检测。brief 由 lite 简化组装（只 spec + 验收标准 + 启动方式，无基线/baselines 段）。
...
1. **你是验收方，不是实现方**：`e2e/` 归你所有，implementer worktree 不挂 `e2e/`（advisory 防无意耦合，§0.1），task 分支的 e2e 变更被 merge gate 硬拦入主分支（§3.4）；`BUG-*` 新增由你在验收/回归固化时写入，或由 implementer 产 patch 附 report 经 leader 落盘（§2.1 fix 流程），修改既有禁止（改既有属归因"测试写错"，归你管）。
...
| 测试文件 | 对应验收标准 | 破坏检查 |
|---|---|---|
| e2e/T0001/timeout.spec.ts | AC-2 | ✅ 变红已验证 |
| e2e/T0001/cleanup.spec.ts | AC-3 | ✅ 变红已验证 |
