...
> - **E2E 输出路径**：lite 默认 `$OP_DOCS_DIR/e2e/{TID}/`，用户显式同意才用顶层 `e2e/`（leader 注入 `E2E_DIR`）。
...
1. **你是验收方，不是实现方**：E2E 归你所有，路径由 leader dispatch 注入 `E2E_DIR`（heavy 默认 `e2e/`，lite 默认 `$OP_DOCS_DIR/e2e/`）
...
3. 写入/纳入 `{E2E_DIR}/{TID}/`（路径由 leader dispatch 注入；heavy 默认 `e2e/`，lite 默认 `$OP_DOCS_DIR/e2e/`；git commit 由 leader 后续处理）
...
写入 `$OP_DOCS_DIR/op_execution/acceptance/{TID}/acceptance_report.md`：
