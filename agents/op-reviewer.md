...
> ```
> **lite 分支**（`OP_PROFILE=lite`）：无 blueprint `test.md`——测试可信判定用内联最小集：①测的是 AC 声明的可观察行为，非内部实现/mock ②断言针对用户可见效果（界面/接口/存储）③无危险模式（永真断言、注释掉断言、mock 掉被测逻辑）④implementer 没偷跑 e2e。
...
1. 读 `docs/omni_powers/op_execution/tasks/{TID}/brief.md`（任务卡 + 指向 spec 路径）→ 读工作 spec（`docs/omni_powers/op_execution/specs/{前缀}.md`）
