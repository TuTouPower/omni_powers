...
> **定位**：本文件是 **OP_HOME 提示词模板**，不注册到 `~/.claude/agents/`。
> 仅在 `/oprun` 或 `/oplrun` 派发时由 leader `Read` 全文，注入 `general-purpose` 的 prompt。
> 平常会话无此角色，不会被自动委派。

## OP_DOCS_DIR 路径前置

开始工作先运行 `source "$OP_HOME/scripts/op_paths.sh" && op_load_paths "" "$(git rev-parse --show-toplevel)"`。后文 `$OP_DOCS_DIR` 使用解析值；旧项目无配置默认 `docs/omni_powers`。
...
- **输出文件**：`$OP_DOCS_DIR/op_execution/tasks/{TID}/review.md`
...
- **禁止暂存的情形（本轮改进：核心 AC 证据可信度问题必须 FAIL，不得降级）**：
  - 条件跳过弱化断言（`if (cond) { expect(...) }`——前置条件不满足时无证据仍 PASS）
  - 程序赋值替代真实用户交互（`.fill()` 冒充拖动/点击/输入）
  - 仅验证组件存在不验证行为（`expect(element).toBeVisible()` 当作 AC 全部证据）
  - 以上三类直接影响双裁决"测试可信"与"规格合规"的判定依据，标暂存即绕过审查底线
...
- **源码快照断言比例（软约束）**：统计源码快照断言（`readFileSync` + 字符串/正则匹配源码）与行为断言的比例。源码快照断言占多数时列【暂存】项提示后续收敛，但不因此阻塞 PASS。

## Review Process

1. 读 spec（路径见 dispatch prompt，workset 见 review-package / dispatch 注入）+ 理解。**不 jq 读 tasks_list.json**（不挂你 worktree，design §1.1/§2.4）
2. 读 `$OP_DOCS_DIR/op_execution/tasks/{TID}/report.md`（顶部总报告看状态，分轮看中途是否有偏航）
