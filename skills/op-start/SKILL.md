...
```bash
cat ~/.claude/teams/op-{project}-team/config.json | jq '.members[] | select(.name == "coder-1")'
# 有结果 → SendMessage 唤醒
# 无结果 → spawn
```
...
| 名称 | 类型 | 数量 | spawn 命令 |
|---|---|---|---|
| coder-1/2/3 | Team 成员 | 1-3 | `Agent({ name: "coder-1", team_name: "op-{project}-team", subagent_type: "op-coder", model: "haiku", prompt: "就绪，等待 leader 派 task。" })` |
...
| task-splitter | Sub Agent | 按需 | `Agent({ name: "splitter", subagent_type: "op-task-splitter", model: "sonnet", prompt: "cd ... && pwd\n拆分..." })` |

> closer 和 task-splitter 无 `team_name`，同步返回，只在对应步骤中临时 spawn。

并发扩展：查 config 确认不存在后 `Agent({ name: "coder-2", team_name: "op-{project}-team", subagent_type: "op-coder", model: "haiku", prompt: "就绪。" })`。
...
| 存在 status=审阅中/进行中 | 进入循环，先扫标记（子步骤 3.1） |
...
选 task（4 条全满足，取 ID 最小）：status=待开始、depends_on 全部完成、不在阻塞范围、ID 最小。

层宽 1 → 串行。层宽 > 1 → 同层并发，上限 3（>3 取 ID 升序前 3，其余等下波次）。

### 2.3 拆 task（按需）

读 plan 发现"多个独立交付单元，各自需独立 review + 能独立回滚"→ 拆。

1. leader 定边界（哪些 step 归 A、哪些归 B、依赖关系）
2. 派 sub agent 执行：

```js
Agent({ name: "splitter", subagent_type: "op-task-splitter", model: "sonnet", prompt: "cd <project_root> && pwd\n拆分 T{n}，参数：[step范围、依赖关系]。" })
```

3. splitter 回报后，leader 按新 tasks_list 回到步骤二

**不能等 coder 写一半再拆**——已落盘代码回切会乱。
...
> closer 和 task-splitter 是 sub agent（同步返回），不需要标记文件。
...
进入循环
    ...
    ▼
  扫标记文件
  for TID in 进行中+审阅中: bash skills/op-start/scripts/op-scan-signals.sh {TID}
    │
    ├─ 有 coder_done → 删标记文件 → 进入子步骤 3.3（派 review）
    │
    ├─ 有 reviews_done → 删两标记文件 → 读 verdict
    │     ├─ 最后一条 verdict 为 PASS → 进入子步骤 3.5（收口）
    │     └─ 最后一条 verdict 为 FAIL → 进入子步骤 3.6（FAIL 轮）
    │
    └─ 皆无标记
...
波次内按 TID 升序分配 coder-1/2/3，并发时每个 coder 独立 worktree。
...
```js
SendMessage({ to: "coder-1", message: "cd <project_root>/.worktrees/{TID} && pwd\n在此目录 TDD 实现 T{n} step {N}。spec: docs/op_execution/tasks/{TID}/spec.md（相关段）。plan: docs/op_execution/tasks/{TID}/plan.md（当前 step）。完成后 touch coder_done 标记文件。" })
```
...
扫到 `coder_done` 后立即派 review（不等整波次完成，先到先审）：
...
每个 task 独立收口。并发波次按依赖顺序合入。leader 串行执行以下 5 小步：
