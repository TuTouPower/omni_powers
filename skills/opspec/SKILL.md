...
把需求转成工作 spec。**内部 skill，由 opintake 在 Stage 0-2 调用（spec 编写是 Stage 1）**。opintake 已完成入口分拣（§1 三判据命中）、和用户澄清了需求、确定了方案——本 skill 把结论落盘成结构化 spec，供闸门 A 审批。
...
feature: <功能名>    # 对应 op_blueprint/specs/{功能名}.md；closer 合入 baseline 时按此映射（临时区按前缀→合入区按功能名）
---
# {名称}
## 一句话意图
{opintake 传入的需求意图}
...
- AC-1 验收信号: {结构化优先——DOM 文本/a11y tree/CLI stdout/API 响应/DB 查询/进程健康检查；视觉对照——截图，advisory}，关键入口: {URL / 菜单路径 / API 端点 / DB 连接 / 进程端口 / 消息队列}
- AC-N 测试缝: {如"需测试收件箱 → 需 Mailpit + seed 账号"; "需验证导出 → 需 stats export 命令"; "需验证定时任务 → 需触发后查 DB 副作用"}
...
## 执行图
{opintake 在 Stage 2 拆完 task 后追加：task 依赖概览 + 接口 task 位置，≤10 行}
```
...
> 信号③（implementer BLOCKED 或两轮 review 不过）属执行期兜底，不在此 skill 范围。由 leader 现场插设计 task（oprun 驱动）。
