...
> **profile 感知**：`docs/omni_powers/profile` = `lite` 时——下文所有 `op_blueprint/`（domain.md/specs//architecture.md）对照步骤跳过（lite 下为空壳占位，不作契约源）；spec 须自足（必要的架构/命名约束内联进 spec 本体）。
...
`docs/omni_powers/op_execution/specs/{TID}_{slug}.md`（本文档路径统一带根 `docs/omni_powers/`）。
...
- 预期失败模式（每条验收标准至少 1 条反例——如果 xxx 没做好，验收标准应该 FAIL）:
...
2. 选模板（spec 模板）
3. 落盘 docs/omni_powers/op_execution/specs/{TID}_{slug}.md（status: draft）
...
4. **验收标准可翻译**：每条 Then 都能翻译为可执行断言？用户可观察？
...
| 文件 | 用途 |
|---|---|
| `docs/omni_powers/op_blueprint/domain.md` | 全局不变量，不变量须对照不冲突 |
| `docs/omni_powers/op_blueprint/specs/` | 生效规格，验收标准须对照不重复 |
