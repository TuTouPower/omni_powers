...
Agent({ name: "coder-2", subagent_type: "harness-coder", model: "haiku", prompt: "..." })
Agent({ name: "coder-3", subagent_type: "harness-coder", model: "haiku", prompt: "..." })
```
...
3. isActive=true → SendMessage 唤醒，确认存活
4. isActive=false 或不存在 → 清残留 → 按需 spawn（显式传 model 参数）
