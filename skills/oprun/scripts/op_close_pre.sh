#!/usr/bin/env bash
# op_close_pre：per-task 收口前机械步骤（标 status=closing）
...
bash "$PLUGIN_ROOT/scripts/op_status.sh" "$TID" closing || die "更新状态失败: $TID → closing"

echo "[OK] close pre: $TID（status=closing）"
