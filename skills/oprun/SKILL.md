...
name: oprun
description: >
  从 checkpoint 续跑：task 循环（review → merge → per-task 验收 → closer 收尾 → 闸门 C → 归档）。
  触发：/oprun、继续、下一步、干活。
...
判定（3.4）
  ...
  │                      └─ PASS ──▶ closer 一段式收尾（3.7）─▶ 闸门 C（3.8）─▶ 归档 ─▶ 回 3.1
  └─ 任一 FAIL
...
呈报四样给人审：验收报告 + spec 变更决策表（从 decisions.md 提取 spec-delta，逐条附理由）+ P0/P1 issue（P0 默认阻断；若用户显式豁免，记录 decisions）+ closer 的 per-task 收尾提案。

人批 → leader 执行实际写入 `op_blueprint/`（specs + baselines 合入）→ 跑归档脚本：
