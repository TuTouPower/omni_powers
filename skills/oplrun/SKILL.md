...
```bash
bash "$OP_HOME/scripts/op_jq.sh" pending
bash "$OP_HOME/scripts/op_jq.sh" deps {TID}      # 确认前置全 done
```
...
```bash
bash "$OP_HOME/scripts/op_read_verdict.sh" {TID}   # exit 0=PASS, 1=FAIL
```
...
bash "$OP_HOME/scripts/close_check.sh" {TID}   # 非 0 不许进下一个 task
```
...
| 文件 | 用途 |
|---|---|
| `$OP_HOME/scripts/op_jq.sh` | tasks_list 查询 |
| `$OP_HOME/scripts/op_status.sh` | 状态流转（lite 枚举，无 closing） |
| `$OP_HOME/scripts/op_implementer_check.sh` | implementer 模式判定 |
| `$OP_HOME/scripts/op_read_verdict.sh` | verdict 读取 |
| `$OP_HOME/scripts/op_close_post.sh` | 收口机械步骤（无 closing，自探测同目录 op_status） |
| `$OP_HOME/scripts/op_assemble_eval_brief.sh` | per-task 裸评 evaluator brief 组装（lite 裸评版） |
| `$OP_HOME/scripts/close_check.sh` | 收口验收 |
| `$OP_HOME/scripts/op_check_env.sh` | 环境检查（jq/git） |
