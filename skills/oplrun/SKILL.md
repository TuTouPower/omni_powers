...
**上下文水位检查（G1）**：每 3 个 task 自检——若 leader 上下文逼近阈值，提示用户 `/compact` 或建议转 heavy。**不能默默跑到失能**。
...
- 验收 **PASS** → leader 归档叶子：spec 原文移 `op_record/specs/`、acceptance 工作区移 `op_record/acceptance/{前缀}/`，前缀标完成（永不复用）。完结报告。
