...
description: >
  ...
  触发：/oplrun、继续、下一步、干活。
  与 heavy 的 oprun 区别：无 hook（leader 亲自验证代替）、无 closer（leader 收口）、脚本自带、Stage 4 裸评。
---
...
leader 先建 task 工作区 + 写 brief：

```bash
mkdir -p docs/omni_powers/op_execution/tasks/{TID}
# 写 brief.md（见下），report.md/review.md 由 agent 产出
...
```
{tasks_list 本 task 完整记录：AC/INV/depends_on/workset}
...
Agent(subagent_type="op-implementer", prompt:
  "cd <项目根> && pwd
   ...
   读 tasks/{TID}/brief.md + 指向的 spec。TDD 实现（先写映射 AC 的结构层单测，不跑 e2e）。
...
**上下文水位流程门（G1，硬约束）**：每 3 个 task 自检 leader 上下文水位——逼近阈值（初始建议 70%，如 200K 窗口→140K）时第一次提示 `/compact` 或转 heavy；**连续 2 个 task 自检超阈值则升级流程门：暂停循环**，呈报用户三选一（compact 后续跑 / 拆当前 spec 回 oplintake / 显式承担风险续跑——选三须 leader append decisions.md，来源标记 `leader-close`）。**不能默默跑到失能**——提示不阻断会滑入静默失能，流程门强制人在环。
...
Agent(subagent_type="op-reviewer", prompt:
  "cd <项目根> && pwd
   环境：OP_PROFILE=lite OP_SCRIPT_ROOT=<oplrun skill 目录>
   review {TID}。读 brief → spec。读 report.md。代码变更：git diff。
   输出 tasks/{TID}/review.md。
   双裁决：①规格合规（覆盖 AC/不偏航）②测试可信（测的是 AC 还是 mock/断言用户可观察/危险模式）。
...
git add {workset}          # brief.md 里本 task 的 workset 路径（src/... 与新增测试）
...
## Stage 4：spec 级验收（整份 spec 所有 task 闭环后）

整份 spec 的所有 task 都 status=完成后，派 op-evaluator 做 spec 级裸评。

leader 先机械组装 brief（不参与内容，防污染）：

```bash
bash "$SCRIPTS/op_assemble_eval_brief.sh" {前缀}
...
Agent(subagent_type="op-evaluator", prompt:
  "cd <项目根> && pwd
   环境：OP_PROFILE=lite OP_SCRIPT_ROOT=<oplrun skill 目录>
   读 docs/omni_powers/op_execution/acceptance/{前缀}/eval_brief.md，按 brief 执行 spec 级裸评 {前缀}。
   逐 AC 评估 → PASS 的 AC 固化成 e2e/{前缀}/ → 破坏检查 → 对抗探索。
   输出 acceptance/{前缀}/eval.md，末行 verdict: PASS 或 FAIL。")
```

> **lite 裸评退化**（§9）：无 worktree 隔离、无 baseline 对照、无跨迭代回归，只做首次裸评。evaluator 能读到 src/——防"抄实现"底线失效，属 lite 已接受代价。

判定：

- 验收范围内 **FAIL** → 转修复 task 回流（走 task 循环）重验，**≤3 轮**（到顶：Critical 升级人裁/转设计 task，Important/Minor 落 issues）。
- 验收 **PASS** → **P0 阻断检查**（代闸门 C 的 P0 阻断语义，design §5.8）：
  ...
  - 无 open P0 → leader 归档叶子：spec 原文移 `op_record/specs/`、acceptance 工作区移 `op_record/acceptance/{前缀}/`，前缀标完成（永不复用）。完结报告。

> lite 无闸门 C、无 closer per-leaf 提案——leader 直接归档（无 blueprint 合入，因 lite 无 blueprint 真相源）。
...
| `scripts/op_assemble_eval_brief.sh` | Stage 4 evaluator brief 组装（lite 裸评版） |
| `scripts/close_check.sh` | 收口验收 |
| `scripts/op_check_p0.sh` | P0 阻断检查（Stage 4 PASS 后归档前扫 open P0） |
