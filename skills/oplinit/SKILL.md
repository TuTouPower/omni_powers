...
`/oplinit` 在目标项目初始化 lite 工作流骨架。**一次性**。零项目侵入——只建 `docs/omni_powers/` 自己的子目录 + 项目根 `e2e/`，不碰宿主任何已有文件。
...
- 建 `docs/omni_powers/` 三区（op_blueprint 空壳 / op_execution / op_record）+ 项目根 `e2e/`
